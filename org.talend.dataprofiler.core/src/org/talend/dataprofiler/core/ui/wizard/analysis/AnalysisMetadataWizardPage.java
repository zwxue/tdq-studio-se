...
public class AnalysisMetadataWizardPage extends MetadataWizardPage {
	...
	private final String pageTitle = DefaultMessagesImpl
			.getString("AnalysisMetadataWizardPage.newAnalysis"); //$NON-NLS-1$

	private final String pageMessage = DefaultMessagesImpl
			.getString("AnalysisMetadataWizardPage.addsAnalysis"); //$NON-NLS-1$
	...
	protected void createExtendedControl(Composite container) {
		...
		typeLabel.setText(DefaultMessagesImpl
				.getString("AnalysisMetadataWizardPage.type")); //$NON-NLS-1$

		typeText = new Text(container, SWT.BORDER);
		AnalysisParameter parameter = (AnalysisParameter) getParameter();
		typeText.setText(parameter.getAnalysisTypeName() != null ? parameter
				.getAnalysisTypeName() : ""); //$NON-NLS-1$
	...
	protected IFolder getStoredFolder() {
		return ResourceManager.getDataProfilingFolder().getFolder(
				DQStructureManager.ANALYSIS);
	}
