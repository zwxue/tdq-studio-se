...
public class ColumnsSelectionDialog extends TwoPartCheckSelectionDialog {
	...
	public ColumnsSelectionDialog(
			AbstractAnalysisMetadataPage metadataFormPage, Shell parent,
			String title, List<Column> columnList, String message) {
	...
	private void initCheckedColumn(List<Column> columnList) {
		...
		for (int i = 0; i < columnList.size(); i++) {
			...
			ColumnSet columnSetOwner = ColumnHelper
					.getColumnSetOwner(columnList.get(i));
			...
			ColumnCheckedMap columnCheckedMap = columnSetCheckedMap
					.get(columnSetKey);
	...
	protected void addCheckedListener() {
		...
		getTreeViewer().addCheckStateListener(new ICheckStateListener() {
			...
			public void checkStateChanged(CheckStateChangedEvent event) {
				...
				ColumnSelectionViewer columnViewer = (ColumnSelectionViewer) event
						.getSource();
				...
				if (event.getChecked()) {
					...
					if (event.getElement() instanceof ColumnSet) {
						...
						handleColumnsChecked((ColumnSet) event.getElement(),
								true);
					}
		...
		getTableViewer().addCheckStateListener(new ICheckStateListener() {
			...
			public void checkStateChanged(CheckStateChangedEvent event) {
				if (event.getElement() instanceof TdColumn) {
					handleColumnChecked((TdColumn) event.getElement(), event
							.getChecked());
				}
			}
		});
	...
	private TdColumn[] getCheckedColumns(ColumnSet columnSet) {
		...
		if (columnCheckMap == null) {
			...
			return columnCheckMap.getCheckedColumns(ColumnSetHelper
	...
	private void handleColumnChecked(TdColumn column, Boolean checkedFlag) {
		...
		ColumnCheckedMap columnCheckMap = columnSetCheckedMap
				.get(new ColumnSetKey(columnSetOwner));
		if (columnCheckMap != null) {
	...
	private void handleColumnsChecked(ColumnSet columnSet, Boolean checkedFlag) {
		...
		if (columnCheckMap != null) {
			...
			columnCheckMap.putAllChecked(EObjectHelper.getColumns(columnSet),
					checkedFlag);
		} else {
			columnCheckMap = new ColumnCheckedMap();
			columnCheckMap.putAllChecked(EObjectHelper.getColumns(columnSet),
	 ...
	 * 
	 * @seeorg.talend.dataprofiler.core.ui.dialog.TwoPartCheckSelectionDialog#
	 * addSelectionButtonListener(org.eclipse.swt .widgets.Button,
		...
		SelectionListener listener = new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (getTableViewer().getInput() != null) {
					handleColumnsChecked((ColumnSet) getTableViewer()
							.getInput(), true);
				}
				updateOKStatus();
			}
	...
	class ColumnSetKey {
		...
		public ColumnSetKey(ColumnSet columnSetOwner) {
			this.catalogName = EObjectHelper.getParent(columnSetOwner)
					.getName();
		...
		public int hashCode() {
			...
			result = prime * result
					+ ((columnSetName == null) ? 0 : columnSetName.hashCode());
			return result;
		}
		...
		public TdColumn[] getCheckedColumns(List<TdColumn> columnList) {
			...
			for (TdColumn column : columnList) {
				if (columnNameMap.containsKey(column.getName())
						&& columnNameMap.get(column.getName())) {
	...
	private List<TdColumn> getAllCheckedColumns() {
		...
		for (int i = 0; i < checkedNodes.length; i++) {
			...
			ColumnSetKey columnSetKey = new ColumnSetKey(
					(ColumnSet) checkedNodes[i]);
			if (columnSetCheckedMap.containsKey(columnSetKey)) {
				ColumnCheckedMap columnMap = columnSetCheckedMap
						.get(columnSetKey);
				columnList.addAll(columnMap
						.getCheckedColumnList((ColumnSet) checkedNodes[i]));
			} else {
				columnList.addAll(ColumnSetHelper
						.getColumns((ColumnSet) checkedNodes[i]));
			}
		}
		return columnList;
	}
	...
	class ColumnLabelProvider extends LabelProvider {
		...
		public String getText(Object element) {
			...
			return columnObj.getName() + PluginConstant.SPACE_STRING
					+ PluginConstant.PARENTHESIS_LEFT
					+ columnObj.getSqlDataType().getName()
			...
			if (inputElement instanceof ColumnSet) {
				...
				if (columnSet != null) {
					...
					if (columns.length <= 0) {
						...
						try {
							List<TdColumn> columnList = DqRepositoryViewService
									.getColumns(provider, columnSet, null, true);
							columns = columnList
									.toArray(new TdColumn[columnList.size()]);
		...
		public Object getParent(Object element) {
			if (element instanceof EObject) {
				...
				ColumnSet columnSet = SwitchHelpers.COLUMN_SET_SWITCH
						.doSwitch(eObj);
				if (columnSet != null) {
					IFolderNode folderNode = FolderNodeHelper.getFolderNode(
							EObjectHelper.getParent((ColumnSet) element),
							columnSet);
					return folderNode;
				}
