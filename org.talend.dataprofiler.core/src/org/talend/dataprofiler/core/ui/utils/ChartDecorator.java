...
public final class ChartDecorator {
	...
	public static void decorate(JFreeChart chart) {
		if (chart != null) {
			...
			if (plot instanceof CategoryPlot) {
				...
				int rowCount = chart.getCategoryPlot().getDataset()
						.getRowCount();

				for (int i = 0; i < rowCount; i++) {
					((CategoryPlot) plot).getRenderer().setSeriesPaint(i,
							colorList.get(i));
				}
			...
			if (plot instanceof XYPlot) {
				...
				for (int i = 0; i < count; i++) {
					((XYPlot) plot).getRenderer().setSeriesPaint(i,
							colorList.get(i));
				}
			}
		}
	...
	public static enum IndiBindColor {
		INDICATOR_ROW_COUNT("Row Count", COLOR_7), INDICATOR_NULL_COUNT(
				"Null Count", COLOR_2), INDICATOR_DISTINCT_COUNT(
				"Distinct Count", COLOR_0), INDICATOR_UNIQUE_COUNT(
				"Unique Count", COLOR_1), INDICATOR_DUPLICATE_COUNT(
				"Duplicate Count", COLOR_3);
