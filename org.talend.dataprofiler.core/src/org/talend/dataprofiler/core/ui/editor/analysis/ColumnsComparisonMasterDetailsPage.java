...
public class ColumnsComparisonMasterDetailsPage extends
	...
	private void createAnalyzedColumnSetsSection(Composite parentComp) {
		...
		createConnBindWidget(columnComp);
		// ~
		...
		if (isEmpty1 && isEmpty2) {
			...
			if (columnSet != null && columnSet.size() != 0) {
				tdProvider = DataProviderHelper.getTdDataProvider(columnSet
						.get(0));
				setConnectionState(tdProvider);
			}
		}
	}

	private void setConnectionState(TdDataProvider tdProvider) {
		String prvFileName = PrvResourceFileHelper.getInstance()
				.findCorrespondingFile(tdProvider).getName();
		Object value = getConnCombo().getData(prvFileName);
		Integer index = 0;
		if (value != null && value instanceof Integer) {
			index = (Integer) value;
		}
		getConnCombo().select(index);
		getConnCombo().setEnabled(false);
	}
	...
	protected ReturnCode canSave() {
		...
		if (columnListA.size() > 0) {
			...
			if (!ColumnHelper.isFromSameTable(columnListA)
					|| !ColumnHelper.isFromSameTable(columnListB)) {
				return new ReturnCode(
						DefaultMessagesImpl
			...
			if (ColumnHelper.isFromSameTable(allColumns)) {
				return new ReturnCode(
						DefaultMessagesImpl
								.getString("ColumnsComparisonMasterDetailsPage.TwoSideColumns"), false); //$NON-NLS-1$
			}
		}
