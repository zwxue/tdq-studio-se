...
public class ColumnsComparisonMasterDetailsPage extends
			...
			if (!DefinitionHandler.getInstance().setDefaultIndicatorDefinition(
				...
				log
						.error(DefaultMessagesImpl
								.getString("ColumnsComparisonMasterDetailsPage.cannotSetIndicatorDef") + indicators[i].getName()); //$NON-NLS-1$
			}
		}
	}
	...
	private void createAnalyzedColumnSetsSection(Composite parentComp) {
		...
		checkComputeButton.setText(DefaultMessagesImpl
				.getString("ColumnsComparisonMasterDetailsPage.0")); //$NON-NLS-1$
		checkComputeButton.setToolTipText(DefaultMessagesImpl
				.getString("ColumnsComparisonMasterDetailsPage.1")); //$NON-NLS-1$
	...
	private Section createSectionPart(Composite parentComp,
		...
		Section columnSetElementSection = this.createSection(form, parentComp,
				title, true, null);
		...
		if (columnList.size() > 0) {
			...
			columnHeader
					.setText(DefaultMessagesImpl
							.getString("ColumnsComparisonMasterDetailsPage.element") + tableName); //$NON-NLS-1$
		}
		...
		if (dialog.open() == Window.OK) {
			...
			if (columnSet.size() != 0) {
				...
				columnsElementViewer
						...
						.setText(
								DefaultMessagesImpl
										.getString("ColumnsComparisonMasterDetailsPage.elements") + tableName); //$NON-NLS-1$
			}
		}
	}
	...
	protected void saveAnalysis() throws DataprofilerCoreException {
		...
		setColumnABForMatchingIndicator(rowMatchingIndicatorA, columnListA,
				columnListB);
		setColumnABForMatchingIndicator(rowMatchingIndicatorB, columnListB,
				columnListA);
		...
		if (analysedElements.size() > 0) {
			TdDataProvider tdDataProvider = EObjectHelper
	...
	protected boolean canSave() {
		if (columnListA.size() != columnListB.size()) {
			MessageDialog
					.openError(
							null,
							DefaultMessagesImpl
									.getString("ColumnsComparisonMasterDetailsPage.error"), //$NON-NLS-1$
							DefaultMessagesImpl
									.getString("ColumnsComparisonMasterDetailsPage.columnsSameMessage")); //$NON-NLS-1$
			return false;
		}
		if (columnListA.size() > 0) {
			...
			for (int i = 0; i < columnListA.size(); i++) {
				if (!((TdColumn) columnListA.get(i)).getSqlDataType().getName()
						.equals(
								((TdColumn) columnListB.get(i))
										.getSqlDataType().getName())) {
					MessageDialog
							.openError(
									null,
									DefaultMessagesImpl
											.getString("ColumnsComparisonMasterDetailsPage.error"), DefaultMessagesImpl.getString("ColumnsComparisonMasterDetailsPage.notSameColumnType")); //$NON-NLS-1$ //$NON-NLS-2$
					return false;
				}
				...
				if (i == 0) {
					...
					if ((columnSetOwnerA != ownerA)
							|| (columnSetOwnerB != ownerB)) {
						MessageDialog
								.openError(
										null,
										DefaultMessagesImpl
												.getString("ColumnsComparisonMasterDetailsPage.error"), DefaultMessagesImpl.getString("ColumnsComparisonMasterDetailsPage.notSameElementMessage")); //$NON-NLS-1$ //$NON-NLS-2$
						return false;
					}
				}
			}
			...
			Package parentCatalogOrSchemaA = ColumnSetHelper
					.getParentCatalogOrSchema(columnSetOwnerA);
			Package parentCatalogOrSchemaB = ColumnSetHelper
					.getParentCatalogOrSchema(columnSetOwnerB);
			if (!parentCatalogOrSchemaA.getName().equals(
					parentCatalogOrSchemaB.getName())) {
				MessageDialog
						.openError(
								null,
								DefaultMessagesImpl
										.getString("ColumnsComparisonMasterDetailsPage.error"), DefaultMessagesImpl.getString("ColumnsComparisonMasterDetailsPage.schemaSameMessage")); //$NON-NLS-1$ //$NON-NLS-2$
				return false;
			}
		}
		return true;
	 ...
	 */
	private void setColumnABForMatchingIndicator(
