...
public final class ChartTableFactory {
	...
	public static void addMenuAndTip(final TableViewer tbViewer,
		...
		tbViewer.getTable().addMouseListener(new MouseAdapter() {
			...
			public void mouseDown(MouseEvent e) {
				...
				// ~
				ExecutionLanguage currentEngine = analysis.getParameters()
						.getExecutionLanguage();

				if (e.button == 3 && ExecutionLanguage.JAVA != currentEngine) {
					StructuredSelection selection = (StructuredSelection) tbViewer
							.getSelection();

					ChartDataEntity dataEntity = (ChartDataEntity) selection
							.getFirstElement();
					...
					if (indicator != null) {
						...
						MenuItemEntity[] itemEntities = ChartTableMenuGenerator
								.generate(explorer, analysis, dataEntity);
							...
							item.addListener(SWT.Selection, new Listener() {
								...
								public void handleEvent(Event event) {
									...
									TdDataProvider tdDataProvider = SwitchHelpers.TDDATAPROVIDER_SWITCH
											.doSwitch(analysis.getContext()
													.getConnection());
									String query = itemEntity.getQuery();
									String editorName = indicator.getName();
									CorePlugin.getDefault().runInDQViewer(
											tdDataProvider, query, editorName);
								}
		...
		Listener tableListener = new Listener() {
			...
			public void handleEvent(Event event) {
				...
				case SWT.MouseHover:
					...
					if (item != null) {
						// show tool tip
						ChartDataEntity entity = (ChartDataEntity) item
								.getData();
	...
	public static void createPattern(Analysis analysis,
			MenuItemEntity itemEntity, final PatternTransformer pattTransformer) {
		String language = pattTransformer.getDbmsLanguage().getDbmsName();
		String query = itemEntity.getQuery();
		String regex = pattTransformer.getRegexp(query.substring(query
				.indexOf('=') + 3, query.lastIndexOf(')') - 1));
		IFolder folder = ResourceManager.getLibrariesFolder().getFolder(
				DQStructureManager.PATTERNS);
		new CreatePatternAction(folder, ExpressionType.REGEXP,
				"'" + regex + "'", language).run(); //$NON-NLS-1$ //$NON-NLS-2$
	}
