 ...
 */
public class TableMasterDetailsPage extends AbstractAnalysisMetadataPage
	...
	public void recomputeIndicators() {
		...
		EList<ModelElement> analyzedTables = analysisHandler
				.getAnalyzedTables();
		...
		for (ModelElement element : analyzedTables) {
			...
			Collection<Indicator> indicatorList = analysisHandler
					.getIndicators(tdTable);
			currentTableIndicator.setIndicators(indicatorList
					.toArray(new Indicator[indicatorList.size()]));
			tableIndicatorList.add(currentTableIndicator);
		}
		currentTableIndicators = tableIndicatorList
	...
	protected void createFormContent(IManagedForm managedForm) {
		...
		form.setText(DefaultMessagesImpl
				.getString("TableMasterDetailsPage.tableAna")); //$NON-NLS-1$
		metadataSection.setText(DefaultMessagesImpl
				.getString("TableMasterDetailsPage.analysisMeta")); //$NON-NLS-1$
		metadataSection.setDescription(DefaultMessagesImpl
				.getString("TableMasterDetailsPage.setPropOfAnalysis")); //$NON-NLS-1$
	...
	void createAnalysisTablesSection(final ScrolledForm form,
			Composite anasisDataComp) {
		analysisTableSection = createSection(
				form,
				anasisDataComp,
				DefaultMessagesImpl
		...
		// ~
		Hyperlink tblBtn = toolkit.createHyperlink(topComp, DefaultMessagesImpl
				.getString("TableMasterDetailsPage.selectTable"), SWT.NONE); //$NON-NLS-1$
		...
		ImageHyperlink collapseAllImageLink = toolkit.createImageHyperlink(
				actionBarComp, SWT.NONE);
		collapseAllImageLink.setToolTipText(DefaultMessagesImpl
				.getString("TableMasterDetailsPage.collapseAllTables")); //$NON-NLS-1$
		...
		ImageHyperlink expandAllImageLink = toolkit.createImageHyperlink(
				actionBarComp, SWT.NONE);
		expandAllImageLink.setToolTipText(DefaultMessagesImpl
				.getString("TableMasterDetailsPage.expandAllTables")); //$NON-NLS-1$
	...
	public void openTableSelectionDialog() {
		...
		TablesSelectionDialog dialog = new TablesSelectionDialog(
				this,
				null,
				DefaultMessagesImpl
	...
	void createDataFilterSection(final ScrolledForm form,
		...
		dataFilterSection = createSection(
				...
				anasisDataComp,
				DefaultMessagesImpl
						.getString("TableMasterDetailsPage.dataFilter"), false, DefaultMessagesImpl.getString("TableMasterDetailsPage.editDataFilter")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	void createPreviewSection(final ScrolledForm form, Composite parent) {
		previewSection = createSection(form, parent, DefaultMessagesImpl
				.getString("TableMasterDetailsPage.graphics"), true, //$NON-NLS-1$
		...
		refreshBtn.setText(DefaultMessagesImpl
				.getString("TableMasterDetailsPage.refreshGraphics")); //$NON-NLS-1$
				...
				if (!analysisStatue) {
					boolean returnCode = MessageDialog
							.openConfirm(
									...
									DefaultMessagesImpl
											.getString("TableMasterDetailsPage.string1")); //$NON-NLS-1$

					if (returnCode) {
						new RunAnalysisAction(TableMasterDetailsPage.this)
								.run();
						message.setVisible(false);
					} else {
						createPreviewCharts(form, chartComposite, false);
						message.setText(DefaultMessagesImpl
								.getString("TableMasterDetailsPage.warning")); //$NON-NLS-1$
	...
	public void createPreviewCharts(final ScrolledForm form,
		...
		for (final TableIndicator tableIndicator : this.treeViewer
				.getTableIndicator()) {
			...
			exComp
					.setText(DefaultMessagesImpl
							.getString("TableMasterDetailsPage.table") + table.getName()); //$NON-NLS-1$
				...
				IRunnableWithProgress rwp = new IRunnableWithProgress() {
					...
					public void run(final IProgressMonitor monitor)
						...
						monitor
								.beginTask(
										DefaultMessagesImpl
												.getString("TableMasterDetailsPage.createPreview") //$NON-NLS-1$
												+ table.getName(),
						...
						Display.getDefault().syncExec(new Runnable() {
							...
							public void run() {
								Map<EIndicatorChartType, List<TableIndicatorUnit>> indicatorComposite = CompositeIndicator
										.getInstance()
										.getTableIndicatorComposite(
												tableIndicator);
								for (EIndicatorChartType chartType : indicatorComposite
										.keySet()) {
									List<TableIndicatorUnit> units = indicatorComposite
											.get(chartType);
									if (!units.isEmpty()) {
										final IChartTypeStates chartTypeState = ChartTypeStatesOperator
												.getChartStateTable(chartType,
														units, tableIndicator);

										List<JFreeChart> charts = chartTypeState
												.getChartList();
										if (charts != null) {
	...
	private void addListenerToChartComp(final ChartComposite chartComp,
			...
			public void chartMouseClicked(ChartMouseEvent event) {
				...
				if (event.getTrigger().getButton() == 1
					...
					item.setText(DefaultMessagesImpl
							.getString("TableMasterDetailsPage.what")); //$NON-NLS-1$
	...
	protected ReturnCode canRun() {
		...
		if (tableIndicators == null || tableIndicators.length == 0) {
			return new ReturnCode(DefaultMessagesImpl
					.getString("TableMasterDetailsPage.NoTableAssigned"), false); //$NON-NLS-1$
		}
		for (TableIndicator tableIndicator : tableIndicators) {
			if (tableIndicator.getIndicators().length == 0) {
				return new ReturnCode(
						DefaultMessagesImpl
								.getString("TableMasterDetailsPage.NoIndicatorAssigned"), false); //$NON-NLS-1$
			}
		}
	...
	public void saveAnalysis() throws DataprofilerCoreException {
		...
		if (tableIndicators != null && tableIndicators.length != 0) {
			...
			tdProvider = EObjectHelper.getTdDataProvider(tableIndicators[0]
					.getTdTable());
			analysis.getContext().setConnection(tdProvider);

			for (TableIndicator tableIndicator : tableIndicators) {
				analysisHandler.addIndicator(tableIndicator.getTdTable(),
						tableIndicator.getIndicators());
			}
		} else {
			analysis.getContext().setConnection(null);
			analysis.getClientDependency().clear();
		}
		analysisHandler.setStringDataFilter(dataFilterComp
				.getDataFilterString());
	...
	public void propertyChange(PropertyChangeEvent evt) {
		if (PluginConstant.ISDIRTY_PROPERTY.equals(evt.getPropertyName())) {
			...
			this.analysisHandler
					.setStringDataFilter((String) evt.getNewValue());
		}
	}
