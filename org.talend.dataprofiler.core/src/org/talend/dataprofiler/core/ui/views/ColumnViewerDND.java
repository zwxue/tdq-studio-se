...
public class ColumnViewerDND {
	...
	public static void installDND(final Tree targetControl) {
		...
		DQRespositoryView findView = (DQRespositoryView) activePage
		...
		DropTargetListener dndListener = new TreeDropTargetEffect(targetControl) {
			...
			public void dragEnter(DropTargetEvent event) {
				...
				if (object instanceof IFile) {
					if (FactoriesUtil.UDI.equals(((IFile) object)
							.getFileExtension())) {
			 ...
			 * 
			 * @see
			 * org.eclipse.swt.dnd.TreeDropTargetEffect#dragOver(org.eclipse
	...
	static class PatternReceiver implements ISelectionReceiver {
		...
		// @Override
		public void doDropValidation(DropTargetEvent event,
			...
			boolean is = true;
			Object firstElement = ((StructuredSelection) commonViewer
					.getSelection()).getFirstElement();
			if (firstElement instanceof IFile) {
				IFile fe = (IFile) firstElement;
				if (NewSourcePatternActionProvider.EXTENSION_PATTERN.equals(fe
						.getFileExtension())) {
					Pattern pattern = PatternResourceFileHelper.getInstance()
							.findPattern(fe);
					if (pattern != null
							&& TaggedValueHelper.getValidStatus(pattern)) {
						is = false;
					}
					TreeItem item = (TreeItem) event.item;
					Object viewer = item == null ? null : item.getParent()
							.getData();
					if (viewer instanceof AnalysisColumnSetTreeViewer) {
						String expressionType = DomainHelper
								.getExpressionType(pattern);
						boolean isSQLPattern = (ExpressionType.SQL_LIKE
								.getLiteral().equals(expressionType));
						if (isSQLPattern) {
							is = true;
						}
					}
			...
			if (event.item == null || is) {
				...
				Object data = event.item
						.getData(AnalysisColumnTreeViewer.INDICATOR_UNIT_KEY);
				if (data != null) {
					event.detail = DND.DROP_NONE;
				} else {
		...
		public void drop(DropTargetEvent event, CommonViewer commonViewer,
			...
			StructuredSelection ts = (StructuredSelection) commonViewer
					.getSelection();
			...
			if (ts.iterator() != null) {
				...
				for (IFile fe : al) {
					TreeItem item = (TreeItem) event.item;
					ColumnIndicator data = (ColumnIndicator) item
							.getData(AnalysisColumnTreeViewer.MODELELEMENT_INDICATOR_KEY);
					viewer = (AnalysisColumnTreeViewer) item.getParent()
							.getData(AnalysisColumnTreeViewer.VIEWER_KEY);
					analysis = viewer.getAnalysis();
					IndicatorUnit addIndicatorUnit = PatternUtilities
							.createIndicatorUnit(fe, data, analysis);
	...
	static class ColumnReceiver implements ISelectionReceiver {
		...
		public void doDropValidation(DropTargetEvent event,
			...
			event.detail = DND.DROP_NONE;
			StructuredSelection structuredSelection = (StructuredSelection) LocalSelectionTransfer
					.getTransfer().getSelection();
		...
		public void drop(DropTargetEvent event, CommonViewer commonViewer,
				int index) {
			LocalSelectionTransfer localSelection = LocalSelectionTransfer
					.getTransfer();
			...
			boolean isAnalysisColumnNominalIntervalTreeViewer = control
					.getData() instanceof AnalysisColumnNominalIntervalTreeViewer;

			AbstractColumnDropTree viewer = (AbstractColumnDropTree) control
					.getData();

			StructuredSelection selection = (StructuredSelection) localSelection
					.getSelection();
			...
			while (it.hasNext()) {
				...
				if (next instanceof TdTable) {
					List<TdColumn> columns = ColumnSetHelper
							.getColumns((TdTable) next);
					if (isAnalysisColumnTreeViewer) {
						for (ModelElementIndicator modelElementIndicator : ((AnalysisColumnTreeViewer) viewer)
							...
							if (columns.contains(modelElementIndicator
									.getModelElement())) {
								columns.remove(modelElementIndicator
										.getModelElement());
							}
						}
					} else if (isAnalysisColumnNominalIntervalTreeViewer) {
						List<Column> oriColumns = ((AnalysisColumnNominalIntervalTreeViewer) viewer)
						...
						List<Column> oriColumns = ((AnalysisColumnSetTreeViewer) viewer)
								.getColumnSetMultiValueList();
						for (Column column : oriColumns) {
							if (columns.contains(column)) {
								columns.remove(column);
	...
	/**
	 * 
	 * DOC mzhao Comment method "isSameType". MOD mzhao 9848 2010-01-14,
	 * Allowing drag table.
	 * 
	 * @param model1
	 * @param model2
	...
	static class XmlElementReceiver implements ISelectionReceiver {
		...
		public void doDropValidation(DropTargetEvent event,
			...
			event.detail = DND.DROP_NONE;
			Object firstElement = ((StructuredSelection) LocalSelectionTransfer
		...
		public void doDropValidation(DropTargetEvent event,
			...
			if (firstElement instanceof IFile) {
				...
				if (FactoriesUtil.UDI.equals(fe.getFileExtension())) {
					IndicatorDefinition udi = UDIResourceFileHelper
							.getInstance().findUDI(fe);
					// MOD yyi 2009-09-16
					// Feature :8866
					if (udi != null
							&& (TaggedValueHelper.getValidStatus(udi) | UDIHelper
									.isUDIValid(udi))) {
						is = false;
					}
				}
		...
		public void drop(DropTargetEvent event, CommonViewer commonViewer,
			...
			if (ts.iterator() != null) {
				...
				for (IFile fe : al) {
					...
					try {
						addIndicatorUnits = UDIUtils.createIndicatorUnit(fe,
								data, analysis);
					} catch (Throwable e) {
						log.error(e, e);
						MessageDialog
					...
					if (addIndicatorUnits != null
							&& addIndicatorUnits.length > 0) {
	...
	public static void installDND(final Table targetControl) {
		...
		DropTargetListener dndListener = new TableDropTargetEffect(
				targetControl) {
	...
	static class ColumnReceiverTable implements ISelectionReceiver {
		...
		public void doDropValidation(DropTargetEvent event,
			...
			if (firstElement instanceof TdColumn) {
				...
				AbstractColumnDropTree viewer = (AbstractColumnDropTree) table
						.getData();
