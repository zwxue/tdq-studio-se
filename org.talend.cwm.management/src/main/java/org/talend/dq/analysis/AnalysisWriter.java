...
public class AnalysisWriter {
	...
	public ReturnCode save(Analysis analysis) {
		...
		// save the resource and related resources (when needed, for example
		// when we change the data mining type of a
		// column)
		...
		if (!saved) {
			rc
					.setReturnCode(
							Messages
									.getString(
											"AnalysisWriter.ProblemSavingAnalysis", analysis.getName()), saved); //$NON-NLS-1$
		}
		return rc;
	...
	/**
	 * Method "save" writes the analysis in the given file. If the file already
	 * exists, it is overridden. This method should not be for saving an
	 * analysis already saved. In this case, use save(Analysis).
	 * 
	 * @param analysis
	 *            the analysis to save
	 * @param file
	 *            the file in which the analysis will be save
		...
		if (!checkFileExtension(file)) {
			rc
					.setReturnCode(
							Messages
									.getString(
											"AnalysisWriter.BadFileExtension", file.getFullPath(), VALID_EXTENSION), false); //$NON-NLS-1$
			return rc;
		}
		EMFSharedResources util = EMFSharedResources.getInstance();

		boolean added = util.addEObjectToResourceSet(file.getFullPath()
				.toString(), analysis);
		...
		if (!added) {
			rc
					.setReturnCode(
							Messages
									.getString(
											"AnalysisWriter.AnalysisNotSave", util.getLastErrorMessage()), added); //$NON-NLS-1$
			return rc;
		}
	...
	/**
	 * Method "createAnalysisFile" creates a new file (or overwrite the existing
	 * file) with the analysis data. This method must not be used to update an
	 * existing analysis. The file is created by
	 ...
	 */
	public TypedReturnCode<IFile> createAnalysisFile(Analysis analysis,
			IFolder folder) {
		assert analysis != null;
		TypedReturnCode<IFile> rc = new TypedReturnCode<IFile>();
		String filename = DqRepositoryViewService.createFilename(analysis
				.getName(), FactoriesUtil.ANA);
		...
		if (file.exists()) {
			rc
					.setReturnCode(
							Messages
									.getString(
											"AnalysisWriter.CannotSaveAnalysis", analysis.getName(), filename), false); //$NON-NLS-1$
			return rc;
		}
		...
		if (saved.isOk()) {
			...
			rc
					.setReturnCode(
							Messages
									.getString(
											"AnalysisWriter.FailedToSaveAnalysis", analysis.getName(), filename), false); //$NON-NLS-1$
		}
		return rc;
	}
