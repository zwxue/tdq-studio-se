...
public class TdqAnalysisConnectionPool {
	...
	private static Logger log = Logger
			.getLogger(TdqAnalysisConnectionPool.class);
	 ...
	 * @param tConnection
	 */
	public TdqAnalysisConnectionPool(
			org.talend.core.model.metadata.builder.connection.Connection tConnection) {
	...
	private Connection newConnection() {
		...
		try {
			TypedReturnCode<Connection> trcConn = JavaSqlFactory
					.createConnection(this.getTConnection());
			if (trcConn.isOk()) {
				conn = trcConn.getObject();
				synchronized (this.synchronizedFlag) {
					this.getPConnections().add(
							new PooledTdqAnalysisConnection(conn));
				}
			}
		...
		try {
			...
			int currentDriverMaxConnections = new Float(
					metaData.getMaxConnections()
							* DEFAULT_CONNECTION_NUMBER_OFFSET).intValue();
			synchronized (this.synchronizedFlag) {
	...
	private synchronized boolean isFull() {
		boolean result = true;
		int topLimit = Math.min(this.getMaxConnections(),
				this.getDriverMaxConnections());
		if (topLimit < 1) {
			result = false;
		} else {
	...
	private synchronized Connection findFreeConnection() {
		...
		Enumeration<PooledTdqAnalysisConnection> enumerate = this
				.getPConnections().elements();
		while (enumerate.hasMoreElements()) {
			PooledTdqAnalysisConnection pConn = (PooledTdqAnalysisConnection) enumerate
					.nextElement();
			try {
				if (!pConn.isBusy()) {
			...
			try {
				...
				while (enumerate.hasMoreElements()) {
					...
					log.info("pConn: id=[" + i + "] pid=[" + pConn.hashCode() + "] conn=[" + pConn.getConnection().toString() + "] [closed=" //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
							+ pConn.getConnection().isClosed()
							+ "] busy=[" + pConn.isBusy() + "]"); //$NON-NLS-1$ //$NON-NLS-2$
				}
				if (!hasElement) {
	...
	/**
	 * DOC xqliu TdqAnalysisConnectionPool class global comment. Detailled
	 * comment
	 */
