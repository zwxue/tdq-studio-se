 ...
 * @author scorreia
 * 
 *         This class contains the singleton instance for the default indicator'
 *         definitions.
 */
public final class DefinitionHandler {
	...
	/**
	 * true if a copy from the file in the plugin directory to the workspace
	 * directory is needed.
	 */
	...
	private IndicatorsDefinitions loadFromFile() {
		...
		if (contents == null) {
			log.error("No content found in given resource: "
					+ definitionsFile.getURI());
			return null;
		}
		if (contents.isEmpty()) {
			log.error("No content found in given resource: "
		...
		DefinitionSwitch<IndicatorsDefinitions> catSwitch = new DefinitionSwitch<IndicatorsDefinitions>() {
			...
			@Override
			public IndicatorsDefinitions caseIndicatorsDefinitions(
					IndicatorsDefinitions object) {
				return object;
			}
	...
	private Resource getResourceFromFile() {
		...
		URI uri = URI.createPlatformResourceURI(WORKSPACE_PATH + FILENAME,
				false);
		...
		if (definitionsFile == null) {
			...
			definitionsFile = util.getResourceSet().getResource(
					URI.createFileURI(".." + File.separator + PLUGIN_PATH),
					true);
		}
		if (definitionsFile == null) {
	...
	public boolean restaureDefaultRegexDefinitions() {
		...
		if (origDef != null) {
			IndicatorDefinition origIndDefinition = getIndicatorDefinition(
					(IndicatorsDefinitions) origDef,
					REGULAR_EXPRESSION_MATCHING);
			IndicatorDefinition regexIndDef = this
					.getIndicatorDefinition(REGULAR_EXPRESSION_MATCHING);
			regexIndDef.getSqlGenericExpression().clear();
			regexIndDef.getSqlGenericExpression().addAll(
					origIndDefinition.getSqlGenericExpression());
			return true;
		}
		return false;
	}
	 ...
	 * 
	 * @return the singleton analysis categories (or throws an exception if a
	 *         problem occured)
	 */
	public IndicatorsDefinitions getIndicatorsDefinitions() {
		...
		if (indicatorDefinitions == null) {
			throw new RuntimeException(Messages
					.getString("DefinitionHandler.IndicatorsDefinition")); //$NON-NLS-1$
		}
	 ...
	 * 
	 * @param folder
	 *            the folder where to copy the default resource.
	...
	public Resource copyDefinitionsIntoFolder(File folder) {
		...
		if (EMFUtil.saveResource(resource)) {
			if (log.isInfoEnabled()) {
				log.info("Indicator default definitions correctly saved in "
						+ resource.getURI());
			} else {
				log.warn("Failed to save default indicator definitions in "
						+ resource.getURI());
			}
		}
		return resource;
	...
	public Resource copyDefinitionsIntoFolder(URI destinationUri) {
		...
		if (EMFUtil.saveResource(resource)) {
			...
			log.error("Failed to save default indicator definitions in "
					+ resource.getURI());
	...
	public static DefinitionHandler getInstance() {
		if (instance == null) {
			...
			if (instance.needCopy) {
				instance.copyDefinitionsIntoFolder(URI
						.createPlatformResourceURI(WORKSPACE_PATH, false));
			}
		}
		return instance;
	}
	...
	/**
	 * Method "setDefaultIndicatorDefinition" sets the indicator's default
	 * definition.
	 * 
	 * @param indicator
	 *            the indicator
	...
	/**
	 * Method "saveResource" saves the indicator definitions (in
	 * .Talend.definition file).
	 * 
	 * @return true if no problem
	 */
	public boolean saveResource() {
		return EMFUtil
				.saveSingleResource(this.indicatorDefinitions.eResource());
	}
	 ...
	 * 
	 * @param label
	 *            the label of the definition to get
	...
	private IndicatorDefinition getIndicatorDefinition(
			IndicatorsDefinitions indDefinitions, String label) {
		EList<IndicatorDefinition> definitions = indDefinitions
				.getIndicatorDefinitions();
		for (IndicatorDefinition indicatorDefinition : definitions) {
			if (indicatorDefinition != null
					&& indicatorDefinition.getLabel() != null
	...
	public boolean updateRegex(String dbmsName, String regexpFunction) {
		...
		IndicatorDefinition regexIndDef = this
				.getIndicatorDefinition(REGULAR_EXPRESSION_MATCHING);
		EList<Expression> sqlGenericExpression = regexIndDef
		...
		if (!replaced) {
			...
			Expression createdExpression = BooleanExpressionHelper
					.createExpression(dbmsName, genericSQL);
	...
	private boolean replaceBodyWith(Expression expression, String regexpFunction) {
		expression.setBody(this.getGenericSQL(expression.getLanguage(),
				regexpFunction));
		return true;
	}

	/**
	 * Note: scorreia. All indicator definitions defined in .Talend.definition
	 * file must be implemented here.
	 * 
	 * WARNING: The label of the indicator definition in .Talend.definition must
	 * be exactly the same as the strings used here.
	 */
	private final IndicatorsSwitch<Boolean> indicatorSwitch = new IndicatorsSwitch<Boolean>() {
		...
		private final SchemaSwitch<Boolean> schemaIndicatorSwitch = new SchemaSwitch<Boolean>() {
			 ...
			 * caseCatalogIndicator(org.talend.dataquality
			 ...
			 * 
			 * @seeorg.talend.dataquality.indicators.schema.util.SchemaSwitch#
			 * caseConnectionIndicator(org.talend.dataquality
			 ...
			 * 
			 * @seeorg.talend.dataquality.indicators.schema.util.SchemaSwitch#
			 * caseSchemaIndicator(org.talend.dataquality
			 ...
			 * 
			 * @seeorg.talend.dataquality.indicators.schema.util.SchemaSwitch#
			 * caseTableIndicator(org.talend.dataquality.
			 ...
			 * 
			 * @seeorg.talend.dataquality.indicators.schema.util.SchemaSwitch#
			 * caseViewIndicator(org.talend.dataquality.
		...
		private final ColumnsetSwitch<Boolean> columnIndicatorSwitch = new ColumnsetSwitch<Boolean>() {
			 ...
			 * 
			 * @see
			 * org.talend.dataquality.indicators.columnset.util.ColumnsetSwitch
			 * #caseRowMatchingIndicator(org.talend.
			 ...
			 * @see
			 * org.talend.dataquality.indicators.columnset.util.ColumnsetSwitch
			 * #caseColumnSetMultiValueIndicator(org
			 * .talend.dataquality.indicators
			 * .columnset.ColumnSetMultiValueIndicator)
			 */
			@Override
			public Boolean caseColumnSetMultiValueIndicator(
					ColumnSetMultiValueIndicator object) {
				return setIndicatorDefinition(object,
						"Multiple Column Frequency Table"); //$NON-NLS-1$
			}
			 ...
			 * @see
			 * org.talend.dataquality.indicators.columnset.util.ColumnsetSwitch
			 * #caseWeakCorrelationIndicator(org.talend
			 * .dataquality.indicators.columnset.WeakCorrelationIndicator)
			 */
			@Override
			public Boolean caseWeakCorrelationIndicator(
					WeakCorrelationIndicator object) {
				return setIndicatorDefinition(object,
						"Multiple Column Correlation"); //$NON-NLS-1$
			}
			 ...
			 * @see
			 * org.talend.dataquality.indicators.columnset.util.ColumnsetSwitch
			 * #caseCountAvgNullIndicator(org.talend
			 * .dataquality.indicators.columnset.CountAvgNullIndicator)
			 */
			@Override
			public Boolean caseCountAvgNullIndicator(
					CountAvgNullIndicator object) {
				return setIndicatorDefinition(object,
						"Averaged Multiple Column Frequency Table"); //$NON-NLS-1$
			}
		 ...
		 * caseDefValueCountIndicator(org.talend.dataquality
		 ...
		 * 
		 * @seeorg.talend.dataquality.indicators.util.IndicatorsSwitch#
		 * caseLowFrequencyIndicator(org.talend.dataquality.
		 ...
		 * 
		 * @seeorg.talend.dataquality.indicators.util.IndicatorsSwitch#
		 * casePatternFreqIndicator(org.talend.dataquality.
		 ...
		 * 
		 * @seeorg.talend.dataquality.indicators.util.IndicatorsSwitch#
		 * casePatternLowFreqIndicator(org.talend.dataquality
		 * .indicators.PatternLowFreqIndicator)
		 */
		@Override
		public Boolean casePatternLowFreqIndicator(
				PatternLowFreqIndicator object) {
		 ...
		 * 
		 * @seeorg.talend.dataquality.indicators.util.IndicatorsSwitch#
		 * caseRegexpMatchingIndicator(org.talend.dataquality
		 * .indicators.RegexpMatchingIndicator)
		 */
		@Override
		public Boolean caseRegexpMatchingIndicator(
				RegexpMatchingIndicator object) {
		 ...
		 * 
		 * @seeorg.talend.dataquality.indicators.util.IndicatorsSwitch#
		 * caseSqlPatternMatchingIndicator(org.talend.dataquality
		 * .indicators.SqlPatternMatchingIndicator)
		 */
		@Override
		public Boolean caseSqlPatternMatchingIndicator(
				SqlPatternMatchingIndicator object) {
		 ...
		 * 
		 * @seeorg.talend.dataquality.indicators.util.IndicatorsSwitch#
		 * casePatternMatchingIndicator(org.talend.dataquality
		 * .indicators.PatternMatchingIndicator)
		 */
		@Override
		public Boolean casePatternMatchingIndicator(
				PatternMatchingIndicator object) {
		 ...
		 * 
		 * @seeorg.talend.dataquality.indicators.util.IndicatorsSwitch#
		 * caseSoundexFreqIndicator(org.talend.dataquality.
		 ...
		 * 
		 * @seeorg.talend.dataquality.indicators.util.IndicatorsSwitch#
		 * caseSoundexLowFreqIndicator(org.talend.dataquality
		 * .indicators.SoundexLowFreqIndicator)
		 */
		@Override
		public Boolean caseSoundexLowFreqIndicator(
				SoundexLowFreqIndicator object) {
			return setIndicatorDefinition(object, "Soundex Low Frequency Table"); //$NON-NLS-1$
		}

		@Override
		public Boolean caseDuplicateCountIndicator(
				DuplicateCountIndicator object) {
	...
	private Boolean setIndicatorDefinition(Indicator indicator,
			String definitionLabel) {
		// get the definition
		IndicatorDefinition indicatorDefinition = this
				.getIndicatorDefinition(definitionLabel);
		if (indicatorDefinition == null) {
			return false;
		}
		// else
	...
	public IndicatorCategory getDQRuleIndicatorCategory() {
		CwmResource resource = (CwmResource) this.indicatorDefinitions
				.eResource();
		EObject object = resource.getEObject(DQ_RULE_CATEGORY);
		if (object != null
				&& DefinitionPackage.eINSTANCE.getIndicatorCategory().equals(
						object.eClass())) {
