...
public abstract class Evaluator<T> {
    ...
    private ReturnCode evaluateIndicators(String sqlStatement) {
        ...
        try {
            if (!prepareIndicators()) {
                rc.setReturnCode(Messages.getString("Evaluator.Problem")+messages, false); //$NON-NLS-1$
                return rc;
            }
    ...
    private boolean prepareIndicators() {
        ...
        for (Indicator indic : allIndicators) {
            if (!indic.prepare()) {
            	if(indic instanceof RegexpMatchingIndicatorImpl){
                  messages=((RegexpMatchingIndicatorImpl) indic).getJavaPatternMessage();
            	}
                ok = false;
            }
        }
