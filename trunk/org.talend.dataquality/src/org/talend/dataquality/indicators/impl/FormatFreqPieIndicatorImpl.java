...
/**
 * <!-- begin-user-doc -->
 * An implementation of the model object '<em><b>Format Freq Pie Indicator</b></em>'.
 * <!-- end-user-doc -->
 * <p>
    ...
    public boolean handle(Object data) {
        ...
        try {
            if (data == null || data.toString().trim().equals(PluginConstant.EMPTY_STRING)) {
                ...
                } else if (data.toString().startsWith(PLUS_SIGN + countryCode)
                        && data.toString().equals(
                                PLUS_SIGN + countryCode + PluginConstant.SPACE_STRING + nationalSignificantNumber)) {
                    wellFormInteCount++;
                    valueToFreq.put(this.WELL_FORM_INTE_KEY, wellFormInteCount);
                    setCurrentKey(WELL_FORM_INTE_KEY);
                } else if (data.toString().equals(nationalSignificantNumber)) {
                    wellFormNatiCount++;
