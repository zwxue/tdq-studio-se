...
public class SelectedLocalComparison implements IComparisonLevel {
	...
	protected void createTempConnectionFile() throws ReloadCompareException {
		...
		tempSecondSelectedDataProvider.setComponent(secondSelectedDataProvider
				.getComponent());
		DataProviderWriter.getInstance().saveDataProviderResource(
				tempSecondSelectedDataProvider,
				(IFolder) secondConnectionFile.getParent(),
					...
					if (columnSet1 != null) {
						...
						if (column1 != null) {
							...
							if (findMathedColumn instanceof TdColumn) {
								((TdColumn) rootElement).getTaggedValue()
										.clear();
								...
								((TdColumn) rootElement).getKeyRelationship()
										.clear();
							}
