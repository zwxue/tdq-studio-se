...
public class IndexMigrator {
    ...
    public int run() throws IOException {
        ...
        if (!inputFolder.exists() || !inputFolder.isDirectory()) {
            System.err.println("The path <" + inputFolder + "> does not exist or is not a folder.");
            System.err.println("Usage: java -jar IndexMigrator.jar <inputFolder> <outputFolder(optinal)>");
            return -1;
        }
    ...
    private int regenerate(File inputFolder, File outputFolder) throws IOException {
        ...
        if (status.missingSegments) {
            ...
            if (fieldNames.contains(F_WORD) && fieldNames.contains(F_SYN)) {
                // for "out of the box" indexes, regenerate the index with 2 extra fields ("SYNTERM" and "WORDTERM") for
                // better scoring.
                ...
                // for any other indexes, regenerate with new Analyzer, but no changes to document.
                for (int i = 0; i < searcher.maxDoc(); i++) {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (args.length > 0) {
            ...
            if (args.length > 1) {
                ...
                migration.setInputPath(outputPath);
            } else {
