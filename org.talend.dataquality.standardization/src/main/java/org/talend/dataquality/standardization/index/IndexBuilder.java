...
public class IndexBuilder {
	...
	public boolean initializeIndex(String csvFileToIndex,
			int[] columnsToBeIndexed) throws IOException {
		assert csvFileToIndex != null;
		if (!new File(csvFileToIndex).exists()
				|| !new File(directoryPath).isDirectory()) {
			throw new IOException(csvFileToIndex + " does not exist or"
		...
		CsvReader csvReader = new CsvReader(new java.io.BufferedReader(
		...
		while (csvReader.readRecord()) {
			...
			if (correctedName != null && correctedName.length() > 0) {
				...
				// TODO not sure that we don't need to index the name too.
				// Probably not because of some wrong data in
	...
	private static void addDoc(IndexWriter w, String name, String synonyms,
			String count, String filePath) throws IOException {
		Document doc = new Document();
		Field field = new Field("name", name, Field.Store.YES,
				Field.Index.ANALYZED, TermVector.NO);
		doc.add(field);
		if (synonyms != null && synonyms.length() > 0) {
			doc.add(new Field("alias", synonyms, Field.Store.YES,
					Field.Index.ANALYZED));
		}
		doc.add(new Field("count", count, Field.Store.YES, Field.Index.NO));
		doc
				.add(new Field("filePath", filePath, Field.Store.YES,
						Field.Index.NO));
		// w.addDocument(doc);
