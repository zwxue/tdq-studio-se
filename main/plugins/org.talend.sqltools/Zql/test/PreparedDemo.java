...
public class PreparedDemo {
   ...
   */
    ...
    static void handleInsert(ZInsert ins) throws Exception {
        ...
        if (columns == null) {
            System.out.println("no column names, assuming _col_1" + (nval > 1 ? " to _col_" + nval : ""));
    ...
    static void handleWhere(ZExp e, Hashtable meta) throws Exception {
        ...
        for (int i = 0; i < operands.size(); i++) {
            if (isPreparedColumn((ZExp) operands.elementAt(i))) {
                ...
                if (operands.size() != 2) {
                    throw new Exception("ERROR in where clause ?? found:" + w.toString());
                }
                break;
            }
        }
        ...
        if (prepared != null) { // prepared contains the (raw) column or alias name
            ...
            System.out.println("[" + (tbl == null ? "unknown" : tbl) + "," + prepared + "]");
    ...
    static boolean isPreparedColumn(ZExp v) {
        return (v instanceof ZExpression && ((ZExpression) v).getOperator().equals("?"));
    }
