...
public class MultiColumnAnalysisExecutor extends ColumnAnalysisSqlExecutor {
    ...
    private void instantiateQuery(Indicator indicator) {
        if (ColumnsetPackage.eINSTANCE.getColumnSetMultiValueIndicator().isSuperTypeOf(indicator.eClass())) {
            ...
            initCatalogOrSchema(AnalysisExecutorHelper.findColumnSetOwner(analyzedColumns.get(0)));
    ...
    private void initCatalogOrSchema(ModelElement columnSetOwner) {
        ...
        // ~
        if (pack == null) {
            log.info(Messages.getString("MultiColumnAnalysisExecutor.NOCATALOGORSCHEMAFOUNDFORCOLUMN", columnSetOwner.getName()));//$NON-NLS-1$
        } else {
