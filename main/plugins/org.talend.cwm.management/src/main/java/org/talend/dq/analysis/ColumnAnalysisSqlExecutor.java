 ...
 * @author scorreia
 * 
 *         Generates the SQL queries for each indicator and each column to be
 *         analyzed. Then executes the queries and stores the results.
 */
public class ColumnAnalysisSqlExecutor extends ColumnAnalysisExecutor {
	...
	private static Logger log = Logger
			.getLogger(ColumnAnalysisSqlExecutor.class);

	/*
	 * (non-Javadoc)
	 * 
	 * @see
	 * org.talend.dq.analysis.AnalysisExecutor#createSqlStatement(org.talend
	...
	public String createSqlStatement(Analysis analysis) {
		...
		try {
			...
			Collection<Indicator> leafIndicators = IndicatorHelper
					.getIndicatorLeaves(results);
			...
			for (Indicator indicator : leafIndicators) {
				if (this.continueRun()) {
					if (!createSqlQuery(stringDataFilter, indicator)) {
						log.error(Messages
								.getString("ColumnAnalysisSqlExecutor.CREATEQUERYERROR") + indicator.getName());//$NON-NLS-1$
						// return null;
					}
				}
			}
	 ...
	 * @throws AnalysisExecutionException
	 */
	private TypedReturnCode<String> getCompletedQuery(Indicator indicator,
			TdColumn tdColumn, String colName,
			IndicatorDefinition indicatorDefinition, String language,
			Expression sqlGenericExpression, final EClass indicatorEclass,
			List<String> whereExpression, List<String> rangeStrings,
			DateGrain dateAggregationType) throws AnalysisExecutionException {
		...
		if (indicatorEclass.equals(IndicatorsPackage.eINSTANCE
				.getMedianIndicator())
				|| indicatorEclass.equals(IndicatorsPackage.eINSTANCE
						.getLowerQuartileIndicator())
				|| indicatorEclass.equals(IndicatorsPackage.eINSTANCE
						.getUpperQuartileIndicator())) {
			// TODO scorreia test type of column and cast when needed
			completedSqlString = getCompletedStringForQuantiles(indicator,
					sqlGenericExpression, colName, table, whereExpression);
			if (completedSqlString != null) {
				if (!PluginConstant.EMPTY_STRING.equals(completedSqlString)) {
					whereExpression = duplicateForCrossJoin(completedSqlString,
							whereExpression, tdColumn);
					completedSqlString = addWhereToSqlStringStatement(
							whereExpression, completedSqlString);
				}
			}
		} else if (indicatorEclass.equals(IndicatorsPackage.eINSTANCE
			...
			int topN = indicatorEclass.equals(IndicatorsPackage.eINSTANCE
					.getModeIndicator()) ? 1 : getTopN(indicator);
			if (topN <= 0) {
				topN = TOP_N;
			}

			if (rangeStrings != null) {
				completedSqlString = getUnionCompletedString(indicator,
						sqlGenericExpression, colName, table, whereExpression,
						rangeStrings);
				if (indicatorEclass.equals(IndicatorsPackage.eINSTANCE
						.getModeIndicator())) {
					// get the best row
					completedSqlString = dbms().getTopNQuery(
							completedSqlString, topN);
				}
			} else if (dateAggregationType != null
					&& !dateAggregationType.equals(DateGrain.NONE)
					// MOD scorreia 2008-06-23 check column type (robustness
					// against bug 4287)
					&& Java2SqlType.isDateInSQL(tdColumn.getSqlDataType()
							.getJavaDataType())) {
				// frequencies with date aggregation
				completedSqlString = getDateAggregatedCompletedStringWithoutAlia(
						sqlGenericExpression, colName, table,
						dateAggregationType);
				completedSqlString = addWhereToSqlStringStatement(
						whereExpression, completedSqlString);
				completedSqlString = dbms().getTopNQuery(completedSqlString,
						topN);
			} else { // usual nominal frequencies
				// wrap column name into a function for Pattern Frequency
				// Statistics indicator
				if (indicatorEclass.equals(IndicatorsPackage.eINSTANCE
						.getPatternFreqIndicator())
						|| indicatorEclass.equals(IndicatorsPackage.eINSTANCE
								.getPatternLowFreqIndicator())) {
					// done scorreia: get user defined functions for Pattern
					// Frequency Statistics
					// MOD xqliu 2009-07-01 bug 7818
					// MOD zshen for bug 12675 2010-05-12
					if (Java2SqlType.isNumbericInSQL(tdColumn.getSqlDataType()
							.getJavaDataType())
							|| Java2SqlType.isDateInSQL(tdColumn
									.getSqlDataType().getJavaDataType())) {
						colName = addFunctionTypeConvert(colName);
						// ~12675
					}

					final EList<CharactersMapping> charactersMapping = indicatorDefinition
							.getCharactersMapping();
					TypedReturnCode<String> columnNameWithFunction = getColumnNameWithFunction(
							indicator, colName, language, charactersMapping);
					if (!columnNameWithFunction.isOk()) {
						rt.setOk(false);
						return rt;
					}
					// MOD for TDQ-8600 If the thread can come here mean that
					// tempColName is not null so give the value
					// to colName
					colName = columnNameWithFunction.getObject();
					// ~
				} else if (indicatorEclass.equals(IndicatorsPackage.eINSTANCE
						.getSoundexFreqIndicator())
						|| indicatorEclass.equals(IndicatorsPackage.eINSTANCE
								.getSoundexLowFreqIndicator())) {
					// change table and soundex(column)
					table = dbms().getSoundexFunction(table, colName);
				}

				completedSqlString = dbms()
						.fillGenericQueryWithColumnTableAndAlias(
								sqlGenericExpression.getBody(), colName, table,
								colName);
				completedSqlString = addWhereToSqlStringStatement(
						whereExpression, completedSqlString);
				completedSqlString = dbms().getTopNQuery(completedSqlString,
		...
		} else if (IndicatorsPackage.eINSTANCE.getPatternMatchingIndicator()
				.isSuperTypeOf(indicatorEclass)) {
			// --- handle case of matching pattern count
			TypedReturnCode<List<String>> checkPatterns = checkPatterns(
					indicator, language);
			...
			completedSqlString = replaceVariables(
					sqlGenericExpression.getBody(), colName, table, patterns);
			completedSqlString = addWhereToSqlStringStatement(whereExpression,
					completedSqlString);
		} else {
			// --- handle case of default value count -> create the where clause
			if (IndicatorsPackage.eINSTANCE.getDefValueCountIndicator().equals(
					indicatorEclass)) {
				String defValue = ColumnHelper.getDefaultValue(tdColumn);
				if (defValue == null) {
					traceError(Messages
							.getString(
									"ColumnAnalysisSqlExecutor.NODEFAULTVALUE", colName, indicator.getName()));//$NON-NLS-1$
					rt.setOk(false);
					return rt;
				}
				// need to generate different SQL where clause for each type.
				int javaType = tdColumn.getSqlDataType().getJavaDataType();
				// MOD qiongli 2011-10-31 add single quotation '' for mysql date
				// type.
				if (!Java2SqlType.isNumbericInSQL(javaType)
				// MOD msjian TDQ-10783: varchar type but with number content,
				// we should add single quotation ''
						&& (!isFunction(defValue, table) || StringUtils
								.isNumeric(defValue.trim()))
						|| (Java2SqlType.isDateInSQL(javaType) && SupportDBUrlType.MYSQLDEFAULTURL
			...
			// --- default case
			// MOD msjian 2011-6-14 21809 fixed: the sql of user defined
			// indicator contains "<%=__GROUP_BY_ALIAS__%>"
			// should be considered
			String sqlBody = sqlGenericExpression.getBody();
			if (sqlBody.indexOf(GenericSQLHandler.GROUP_BY_ALIAS) != -1) {
				completedSqlString = dbms()
						.fillGenericQueryWithColumnTableAndAlias(sqlBody,
								colName, table, colName);
			} else {
				completedSqlString = dbms()
						.fillGenericQueryWithColumnsAndTable(sqlBody, colName,
								table);
			}
			completedSqlString = addWhereToSqlStringStatement(whereExpression,
					completedSqlString);
	 ...
	 * @return
	 */
	private TypedReturnCode<String> getColumnNameWithFunction(
			Indicator indicator, String colName, String language,
			final EList<CharactersMapping> charactersMapping) {
		TypedReturnCode<String> rt = new TypedReturnCode<String>(true);
		String colNameWithFunction = dbms().getPatternFinderFunction(colName,
				charactersMapping);
		if (colNameWithFunction == null) {
			colNameWithFunction = dbms().getPatternFinderDefaultFunction(
					colName);
			if (colNameWithFunction == null) {
				traceError(Messages
						.getString(
	 ...
	 * @return
	 */
	private TypedReturnCode<List<String>> checkPatterns(Indicator indicator,
			String language) {
		TypedReturnCode<List<String>> rt = new TypedReturnCode<List<String>>(
				true);
		List<String> patterns = getPatterns(indicator);
		if (patterns.isEmpty()) {
			// Added TDQ-8468 yyin 20131227 : if the used SQL pattern already
			// has its correct expression instance in
			// the analysis, will not check the expression and create again(from
			// the definition).
	 ...
	 * @return
	 */
	private TypedReturnCode<String> getColumnName(Indicator indicator,
			TdColumn tdColumn) {
		TypedReturnCode<String> rt = new TypedReturnCode<String>(true);
		...
		colName = dbms().castColumn4ColumnAnalysisSqlExecutor(indicator,
				tdColumn, colName);
	...
	private TypedReturnCode<TdColumn> getTdColumn(Indicator indicator) {
		...
		if (analyzedElement == null) {
			traceError(Messages
					.getString(
							"ColumnAnalysisSqlExecutor.ANALYSISELEMENTISNULL", indicator.getName()));//$NON-NLS-1$
			rt.setOk(false);
			return rt;
		}

		TdColumn tdColumn = SwitchHelpers.COLUMN_SWITCH
				.doSwitch(analyzedElement);
		rt.setObject(tdColumn);
		if (tdColumn == null) {
			traceError(Messages
					.getString(
							"ColumnAnalysisSqlExecutor.ANALYZEDISNOTCOLUMNINDICATOR", indicator.getName()));//$NON-NLS-1$
			rt.setOk(false);
		}
		return rt;
	}
	 ...
	 * @return
	 */
	private TypedReturnCode<IndicatorDefinition> getIndicatorDefinition(
			Indicator indicator) {
		TypedReturnCode<IndicatorDefinition> rt = new TypedReturnCode<IndicatorDefinition>(
				true);
		...
		if (label == null || PluginConstant.EMPTY_STRING.equals(label)) {
			...
			indicatorDefinition = DefinitionHandler.getInstance()
					.getIndicatorDefinition(label);
		}

		// TDQ-10559: when the indicator is in reference project, the above can
		// get NOTHING
		if (indicatorDefinition == null) {
			indicatorDefinition = indicator.getIndicatorDefinition();
			if (indicatorDefinition.eIsProxy()) {
				indicatorDefinition = (IndicatorDefinition) EObjectHelper
						.resolveObject(indicatorDefinition);
			}
		}
		// TDQ-10559~
		...
		if (indicatorDefinition == null) {
			traceError(Messages
					.getString(
							"ColumnAnalysisSqlExecutor.INTERNALERROR", indicator.getName()));//$NON-NLS-1$
			rt.setOk(false);
		}
	...
	private boolean isFunction(String defValue, String table) {
		...
		try {
			...
			// MOD qiongli 2011-5-20,don't print error in error log view and use
			// finnaly to close Statement.
			try {
				stat.close();
	...
	/**
	 * Method "duplicateForCrossJoin". For some SQL queries, auto-joins are used
	 * in subqueries. This means that the table has two differents aliases and
	 * the columns must be prefixed with the alias of the table. Each where
	 * clause must be duplicated. For example, the clause "AGE > 10" must be
	 * duplicated to give "a.AGE > 10" and "b.AGE" when table aliases are "a"
	 * and "b".
	 * 
	 ...
	 */
	private List<String> duplicateForCrossJoin(String completedSqlString,
			List<String> whereExpression, TdColumn tdColumn) {
		if (whereExpression.isEmpty()) {
			return whereExpression;
		}
		String quotedColName = getQuotedColumnName(tdColumn);
		String[] tableAliases = getTableTableAliasA(completedSqlString,
				quotedColName);
		...
		ColumnSet columnSetOwner = ColumnHelper
				.getColumnOwnerAsColumnSet(tdColumn);
		List<TdColumn> columns = ColumnHelper.getColumns(columnSetOwner
				.getFeature());
		for (String where : whereExpression) {
			...
			for (TdColumn col : columns) {
				String colNameToReplace = where.contains(quotedColName) ? quotedColName
						: col.getName();
				if (where.contains(colNameToReplace)) {
					whereA = whereA.replace(colNameToReplace, tableAliases[0]
							+ PluginConstant.DOT_STRING + colNameToReplace);
					whereB = whereB.replace(colNameToReplace, tableAliases[1]
							+ PluginConstant.DOT_STRING + colNameToReplace);
				}
			}
	 ...
	 * @return
	 */
	private String[] getTableTableAliasA(String completedSqlString,
			String quotedColName) {
		java.util.regex.Pattern p = java.util.regex.Pattern.compile(ALIAS
				+ quotedColName);
	 ...
	 * 
	 */
	private String replaceVariables(String sqlGenericString, String colName,
			String table, List<String> patterns) {
		assert (patterns.size() != 0);
		return dbms().fillGenericQueryWithColumnTablePattern(sqlGenericString,
				colName, table, patterns.get(0));
	}
	...
	/**
	 * 
	 * DOC mzhao bug 12675: Drill down of date type not correct. Here would not
	 * use alias.
	 * 
	 * @param sqlExpression
	 * @param colName
	 ...
	 * @return
	 */
	private String getDateAggregatedCompletedStringWithoutAlia(
			Expression sqlExpression, String colName, String table,
		...
		case NONE:
			result = colName;
			// aliases = colName; // bug 5336 fixed aliases must not be empty
			// otherwise the group by clause is empty.
			break;
		default:
			break;
		}
		// String groupByAliases = dbms().supportAliasesInGroupBy() ? aliases :
		// result;
		String sql = dbms().fillGenericQueryWithColumnTableAndAlias(
				sqlExpression.getBody(), result, table, result);
		return sql;
	}
	...
	private String comma(String previousContent) {
		return (previousContent.length() == 0) ? previousContent : COMMA
				+ previousContent;
	}
	 ...
	 * @throws ParseException
	 */
	private String addWhereToSqlStringStatement(List<String> whereExpressions,
			String completedSqlString) {
		return dbms().addWhereToSqlStringStatement(completedSqlString,
				whereExpressions);
	}
	 ...
	 * @throws ParseException
	 */
	private String getUnionCompletedString(Indicator indicator,
			Expression sqlExpression, String colName, String table,
			List<String> whereExpression, List<String> rangeStrings) {
		StringBuffer buf = new StringBuffer();
		...
		// MOD scorreia 2008-09-03 Bug #4976 Order by clause cannot be in each
		// single select statement (Oracle
		...
		// MOD scorreia 2008-12-16 handle teradata case where "order by" clause
		// needs a number to identify the column
		// by removing the unnecessary "order by" clause
		// String orderBy = (idxOfOrderBY != -1) ?
		// sqlGenericExpression.substring(idxOfOrderBY) : "";
		String singleStatement = (idxOfOrderBY != -1) ? sqlGenericExpression
				.substring(0, idxOfOrderBY) : sqlGenericExpression;

		for (int i = 0; i < last; i++) {
			String singleSelect = getCompletedSingleSelect(indicator,
					singleStatement, colName, table, whereExpression,
					rangeStrings.get(i));
		...
		// MOD scorreia 2008-09-03 Bug #4976 append the order by clause at the
	 ...
	 * @throws ParseException
	 */
	private String getCompletedSingleSelect(Indicator indicator,
			String sqlGenericExpression, String colName, String table,
			List<String> whereExpression, String range) {
		String completedRange = this.unquote(range); // replaceVariablesLow(range,
														// this.unquote(colName),
		// this.unquote(table));
		String rangeColumn = "'" + completedRange + "'"; //$NON-NLS-1$ //$NON-NLS-2$

		String singleQuery = removeGroupBy(sqlGenericExpression);
		String completedSqlString = dbms().fillGenericQueryWithColumnsAndTable(
				singleQuery, rangeColumn, table);

		List<String> allWheresForSingleSelect = new ArrayList<String>(
				whereExpression);

		// add this range clause to the given where clause (but do not modify
		// the given where clause)
		// allWheresForSingleSelect.add(completedRange);

		completedSqlString = addWhereToSqlStringStatement(
				allWheresForSingleSelect, completedSqlString);
		// replacement in order to get lines even when no data is available
		// do this replacement after having added the where clause otherwise the
		// parsing with ZQL will fail.
	...
	private String removeGroupBy(String sqlGenericExpression) {
		int idxOfGroupBy = sqlGenericExpression.toUpperCase().indexOf(
				dbms().groupBy());
		return sqlGenericExpression.substring(0, idxOfGroupBy);
	}

	/**
	 * Method "replaceCountByZeroCount" replaces "COUNT(*)" by
	 * "CASE WHEN completedRange THEN COUNT(*) ELSE 0 END" in the given SQL
	 ...
	 */
	private String replaceCountByZeroCount(String completedSqlString,
			String completedRange) {
		return completedSqlString
				.replace(
						"COUNT(*)", "COUNT( CASE WHEN " + completedRange + " THEN 1 END )"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	}
	...
	private List<String> getBinsAsGenericString(EList<RangeRestriction> ranges,
		...
		for (RangeRestriction rangeRestriction : ranges) {
			String bin = colName + dbms().greaterOrEqual()
					+ DomainHelper.getMinValue(rangeRestriction) + dbms().and()
					+ colName + dbms().less()
					+ DomainHelper.getMaxValue(rangeRestriction);
			// set the name of the RangeRestriction here
			// TODO range name should be set at the construction (in the bins
			// designer wizard)
	 ...
	 * @throws AnalysisExecutionException
	 */
	private String getCompletedStringForQuantiles(Indicator indicator,
			Expression sqlExpression, String colName, String table,
			List<String> whereExpression) throws AnalysisExecutionException {
		// first, count nb lines
		String catalogOrSchema = getCatalogOrSchemaName(indicator
				.getAnalyzedElement());
		long count = getCount(cachedAnalysis, colName, table, catalogOrSchema,
				whereExpression);
		if (count == -1) {
			setError(Messages.getString(
					"ColumnAnalysisSqlExecutor.GotInvalidResultSet", //$NON-NLS-1$
		...
		if (count == 0) {
			...
			return dbms().fillGenericQueryWithColumnTableLimitOffset(
					sqlExpression.getBody(), colName, table, "0", "0", "0");
		}

		Long midleCount = getOffsetInLimit(indicator, count);
		Integer nbRow = getNbReturnedRows(indicator, count);

		long nPlusSkip = midleCount + nbRow; // needed for MSSQL query with TOP
												// clause
		return dbms().fillGenericQueryWithColumnTableLimitOffset(
				sqlExpression.getBody(), colName, table, String.valueOf(nbRow),
	...
	private Integer getNbReturnedRows(Indicator indicator, long count) {
		if (indicator.eClass().equals(
				IndicatorsPackage.eINSTANCE.getMedianIndicator())) {
			boolean isEven = count % 2 == 0;
			return (isEven) ? 2 : 1;
		} else if (indicator.eClass().equals(
				IndicatorsPackage.eINSTANCE.getLowerQuartileIndicator())
				|| indicator.eClass()
						.equals(IndicatorsPackage.eINSTANCE
								.getUpperQuartileIndicator())) {
	 ...
	 * @param indicator
	 * @param count
	 * @return the number of rows to skip in order to compute the quartiles. In
	 *         case of an odd number of rows, one row is returned and the
	 *         returned value is the index of the row before the quartile. In
	 *         case of an even number of rows, two rows are returned and the
	 *         quartile is the average of the two values (for the median).
	 */
	private Long getOffsetInLimit(Indicator indicator, long count) {
		...
		// Be careful, -1 is needed to get the searched index -1 because the
		// searched index is 3 when n=4
		boolean isEven = count % 2 == 0;
		if (indicator.eClass().equals(
				IndicatorsPackage.eINSTANCE.getMedianIndicator())) {
			return isEven ? count / 2 - 1 : (count - 1) / 2;
				...
				IndicatorsPackage.eINSTANCE.getLowerQuartileIndicator())) {
			return /* isEven ? */Math.round(0.25 * (count + 1)) - 1 /*
																	 * :
																	 * Math.round
																	 * (0.25
																	 * count)
				...
				IndicatorsPackage.eINSTANCE.getUpperQuartileIndicator())) {
			Long res = 0L; // count - (Math.round(0.25 * (count + 1))) - 1; does
							// not work
			if (isEven) {
				if ((count / 2) % 2 == 0) {
					res = 3 * count / 4 - 1;
	 ...
	 * @param analysis
	 * @param colName
	 *            the column name should be surrounded by the SQL quotes
	 * @param table
	 *            the table name should be surrounded by the SQL quotes
	 * @param catalog
	 *            the catalog (or schema) name
	 * @param whereExpression
	 * @return
	 * @throws AnalysisExecutionException
	 */
	protected Long getCount(Analysis analysis, String colName, String table,
			String catalog, List<String> whereExpression)
			throws AnalysisExecutionException {
		try {
			return getCountLow(analysis, colName, table, catalog,
					whereExpression);
		} catch (SQLException e) {
			throw new AnalysisExecutionException(Messages.getString(
					"ColumnAnalysisSqlExecutor.CannotGetCount",//$NON-NLS-1$
					analysis.getName(), colName,
					dbms().toQualifiedName(catalog, null, table)), e);
		}
	}
	 ...
	 * @throws AnalysisExecutionException
	 */
	private Long getCountLow(Analysis analysis, String colName, String table,
			String catalogName, List<String> whereExpression)
		...
		List<Object[]> myResultSet = executeQuery(catalogName, connection,
				queryStmt);
		...
		if (myResultSet.isEmpty() || myResultSet.size() > 1) {
			log.error(Messages
					.getString("ColumnAnalysisSqlExecutor.TOOMANYRESULTOBTAINED") + myResultSet);//$NON-NLS-1$  
			return -1L;
		}
		// MOD zshen exchange OracleODBC will get a double value to int.Else
		// Have a NumberFormatException at here.
		return Long
				.valueOf(String.valueOf(myResultSet.get(0)[0]).split("\\.")[0]);//$NON-NLS-1$  
		// ~
	}
	 ...
	 * @return
	 */
	protected String replaceVariablesLow(String sqlGenericString,
			Object... arguments) {
	 ...
	 * 
	 * @see
	 * org.talend.dq.analysis.AnalysisExecutor#runAnalysis(org.talend.dataquality
	 * .analysis.Analysis, java.lang.String)
	 */
	@Override
	protected ReturnCode evaluate(Analysis analysis,
			java.sql.Connection connection, String sqlStatement) {
		boolean ok = true;

		// store map of element to each indicator used for computation (leaf
		// indicator)
		...
		List<Indicator> indicators = IndicatorHelper
				.getIndicatorLeaves(analysis.getResults());

		try {
			if (canParallel(connection)) {
				ok = runAnalysisIndicatorsParallel(analysis,
						elementToIndicator, indicators, POOLED_CONNECTION);
			} else {
				ok = runAnalysisIndicators(connection, elementToIndicator,
						indicators);
			}
			// --- finalize indicators by setting the row count and null when
			// they exist.
	 ...
	 * @throws SQLException
	 */
	private boolean runAnalysisIndicators(Connection connection,
		...
		for (Indicator indicator : indicators) {
			...
			String catalogName = getCatalogOrSchemaName(indicator
					.getAnalyzedElement());
			if (catalogName != null) { // check whether null argument can be
										// given
			...
			if (query == null
					|| !executeQuery(indicator, connection, query.getBody())) {
			...
			MultiMapHelper.addUniqueObjectToListMap(
					indicator.getAnalyzedElement(), indicator,
					elementToIndicator);
		}
		return runStatus;
	}
	...
	static class ExecutiveAnalysisJob extends Job {
		...
		public ExecutiveAnalysisJob(ColumnAnalysisSqlExecutor parent,
				Connection connection,
				Map<ModelElement, List<Indicator>> elementToIndicator,
				Indicator indicator) {
		...
		protected IStatus run(IProgressMonitor monitor) {
			ColumnAnalysisSqlParallelExecutor columnSqlParallel = ColumnAnalysisSqlParallelExecutor
					.createInstance(parent, connection, elementToIndicator,
							indicator);
			...
			if (isSuccess) {
				...
				this.errorMessage = Messages
						.getString("ColumnAnalysisSqlExecutor.AnalysisExecutionFailed"); //$NON-NLS-1$
	...
	private boolean runAnalysisIndicatorsParallel(Analysis analysis,
			Map<ModelElement, List<Indicator>> elementToIndicator,
			List<Indicator> indicators, boolean pooledConnection)
			throws SQLException {
		...
		try {
			...
			for (Indicator indicator : indicators) {
				if (this.continueRun()) {
					...
					if (conn != null) {
						ExecutiveAnalysisJob eaj = new ExecutiveAnalysisJob(
								ColumnAnalysisSqlExecutor.this, conn,
			...
			// Added TDQ-8388 20140530 yyin: only show one message to let the
			// user check detail in error log.
			if (hasErrorMessage) {
				setError(Messages
						.getString("ColumnAnalysisSqlExecutor.ERRORREFERTOLOG"));//$NON-NLS-1$
			}
	 ...
	 * @param elementToIndicator
	 */
	protected void setRowCountAndNullCount(
			Map<ModelElement, List<Indicator>> elementToIndicator) {
		...
		for (ModelElement modelElement : analyzedElements) {
			...
			RowCountIndicator rowCount = IndicatorHelper.getRowCountIndicator(
					modelElement, elementToIndicator);
			// get null count indicator
			NullCountIndicator nullCount = IndicatorHelper
					.getNullCountIndicator(modelElement, elementToIndicator);
	...
	private boolean needPercentage(Indicator ind) {
		...
		return indType != IndicatorEnum.ModeIndicatorEnum
				&& !indType.isAChildOf(IndicatorEnum.TextIndicatorEnum)
	...
	protected String getCatalogOrSchemaName(ModelElement analyzedElement) {
		...
		if (schema == null) {
			if (!isSchemataProxy()) {
				log.error(Messages
						.getString("ColumnAnalysisSqlExecutor.NOSCHEMAFOUNDFORCOLUMN") + " " + ((analyzedElement != null) ? analyzedElement.getName() : "Unknow column"));//$NON-NLS-1$  //$NON-NLS-2$
			}
			return null;
		}
		// else
		if (RelationalPackage.eINSTANCE.getSchema().equals(schema.eClass())) {
			final Catalog parentCatalog = CatalogHelper
					.getParentCatalog(schema);
	 ...
	 * @throws SQLException
	 */
	protected boolean executeQuery(final Indicator indicator,
			Connection connection, String queryStmt) throws SQLException {
		String cat = getCatalogOrSchemaName(indicator.getAnalyzedElement());
		if (log.isInfoEnabled()) {
			log.info(Messages
					.getString(
							"ColumnAnalysisSqlExecutor.COMPUTINGINDICATOR", indicator.getName()) //$NON-NLS-1$ 
		...
		boolean ret = false;
		// MOD qiongli 2012-3-7 TDQ-4632 delete some redundancy code for
		// DistinctIndicator.modify directly the sql
		...
		// Added TDQ-8787 publish the related event when one indicator is
		// finished: to refresh the chart with new result
		// of the current indicator
		final ITDQRepositoryService tdqRepositoryService = AnalysisExecutorHelper
				.getTDQService();
	 ...
	 * @throws SQLException
	 */
	protected List<Object[]> executeQuery(String catalogName,
			Connection connection, String queryStmt) throws SQLException {
		// set current thread classLoader if it is hive connection
		ClassLoader currClassLoader = Thread.currentThread()
				.getContextClassLoader();
		org.talend.core.model.metadata.builder.connection.Connection dbConn = this
				.getAnalysisDataProvider(cachedAnalysis);
		IMetadataConnection metadataBean = ConvertionHelper.convert(dbConn);
		if (EDatabaseTypeName.HIVE.getXmlName().equalsIgnoreCase(
				metadataBean.getDbType())) {
			ClassLoader hiveClassLoader = HiveClassLoaderFactory.getInstance()
					.getClassLoader(metadataBean);
			...
			if (continueRun()) {
				...
				if (resultSet == null) {
					String mess = Messages
							.getString("ColumnAnalysisSqlExecutor.NORESULTSETFORTHISSTATEMENT") + queryStmt;//$NON-NLS-1$  
					log.warn(mess);
					return null;
				}
	...
	protected boolean changeCatalog(String catalogName, Connection connection) {
		try {
			...
			if (!(ConnectionUtils.isOdbcMssql(connection)
					|| ConnectionUtils.isOdbcOracle(connection)
					|| ConnectionUtils.isOdbcProgress(connection)
					|| ConnectionUtils.isOdbcTeradata(connection) || ExtractMetaDataUtils
					.getInstance().isHiveConnection(connection))) {
				// MOD scorreia 2008-08-01 MSSQL does not support quoted
				// catalog's name
			...
			traceError(Messages
					.getString(
							"ColumnAnalysisSqlExecutor.ERRORWHENSETCATALOG", catalogName, e.getMessage()));//$NON-NLS-1$  
			return Boolean.FALSE;
		} catch (SQLException e) {
			traceError(Messages
					.getString(
							"ColumnAnalysisSqlExecutor.ERRORWHENSETCATALOGSQL", catalogName, e.getMessage()));//$NON-NLS-1$
			return Boolean.FALSE;
		}
	}
	...
	private boolean canParallel(Connection connection) {
		try {
			...
			if (connectionMetadata.getDriverName() != null
					&& connectionMetadata.getDriverName().toLowerCase()
							.startsWith(DatabaseConstant.ODBC_DRIVER_NAME)) {
				return false;
			}

			// ADD msjian 2011-6-24 22483 fixed: fixed
