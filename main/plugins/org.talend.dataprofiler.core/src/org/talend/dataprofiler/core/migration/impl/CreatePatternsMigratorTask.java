...
public class CreatePatternsMigratorTask extends AbstractWorksapceUpdateTask {
    ...
    protected boolean doExecute() throws Exception {
        ...
        if (folder.exists()) {
            ...
            pattern = newPattern("MAC Address", SQLLanguage, "'^([0-9a-fA-F][0-9a-fA-F]:){5}([0-9a-fA-F][0-9a-fA-F])$'"); //$NON-NLS-1$ //$NON-NLS-2$
            if (pattern != null) {
        ...
        if (folder.exists()) {
            ...
            pattern = newPattern("UK SSN", SQLLanguage, //$NON-NLS-1$
        ...
        if (folder.exists()) {
            ...
            pattern = newPattern("FR Amount Money", SQLLanguage, //$NON-NLS-1$
        ...
        if (folder.exists()) {
            Pattern pattern = newPattern("EN_Month_Abbrev", SQLLanguage, "'^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)$'"); //$NON-NLS-1$ //$NON-NLS-2$
            if (pattern != null) {
                setTagValue(pattern, "Jan | Feb ", "Month English abbreviation"); //$NON-NLS-1$ //$NON-NLS-2$
                ElementWriterFactory.getInstance().createPatternWriter().create(pattern, folder);
            }

            pattern = newPattern("EN_Month", SQLLanguage, //$NON-NLS-1$
