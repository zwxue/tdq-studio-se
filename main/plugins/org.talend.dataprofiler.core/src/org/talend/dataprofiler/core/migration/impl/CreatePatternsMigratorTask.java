...
public class CreatePatternsMigratorTask extends AbstractWorksapceUpdateTask {
    ...
    private final String SQLLanguage = "SQL";

    private final String PATH_ADDRESS = "address";

    private final String PATH_CUSTOMER = "customer";

    private final String PATH_NUMBER = "number";

    private final String PATH_MONEY = "currency";

    private final String PATH_DATE = "date";

    private final String PATH_INTERNET = "internet";
    ...
    protected boolean doExecute() throws Exception {
        ...
        if (folder.exists()) {
            ...
            if (pattern != null) {
                setTagValue(pattern, " FRA-2A235 |   F-2B128 |  FRA 2B356",
        ...
        if (folder.exists()) {
            Pattern pattern = newPattern("GPS Coordinate", SQLLanguage,
            ...
            // pattern = newPattern("ISBN 13", SQLLanguage,
            // "'^ISBN(?:-13)?:?\\ *(97(?:8|9)([ -]?)(?=\\d{1,5}\\2?\\d{1,7}\\2?\\d{1,6}\\2?\\d)(?:\\d\\2*){9}\\d)$'");
            // if (pattern != null) {
            // setTagValue(pattern, "International Standard Book Number 13 digits",
            // "ISBN-13: 978-2711791415 || ISBN: 978-2711791415");
            // ElementWriterFactory.getInstance().createPatternWriter().create(pattern, folder);
            // }

            pattern = newPattern("UK SSN", SQLLanguage,
                    "'^[A-CEGHJ-PR-TW-Z]{1}[A-CEGHJ-NPR-TW-Z]{1}([0-9]{6}|( [0-9]{2}){3} )[A-DFM]{0,1}$'");
            if (pattern != null) {
                setTagValue(pattern, "AB123456C | AB 12 34 56 C",
        ...
        if (folder.exists()) {
            ...
            pattern = newPattern("FR Amount Money", SQLLanguage,
        ...
        if (folder.exists()) {
            Pattern pattern = newPattern("EN_Month_Abbrev", SQLLanguage, "'^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)$'");
            if (pattern != null) {
                setTagValue(pattern, "Jan | Feb ", "Month English abbreviation");
                ElementWriterFactory.getInstance().createPatternWriter().create(pattern, folder);
            }

            pattern = newPattern("EN_Month", SQLLanguage,
