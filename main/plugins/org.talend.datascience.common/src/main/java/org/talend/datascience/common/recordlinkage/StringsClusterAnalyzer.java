...
public class StringsClusterAnalyzer implements Analyzer<StringsCluster> {
	...
	public void init() {
		...
		blockKeyDefMap.put(MatchAnalysisConstant.KEY_ALGO,
				blockingKeyAlgo.getValue());
		...
		blockKeyHandler = new BlockingKeyHandler(blockKeySchema,
				colName2IndexMap);
		...
		recordGroup = new AbstractRecordGrouping<Object>() {
			 ...
			 * 
			 * @see
			 * org.talend.dataquality.record.linkage.grouping.AbstractRecordGrouping
			 * #isMaster(java.lang.Object)
			 */
			@Override
			 ...
			 * @see
			 * org.talend.dataquality.record.linkage.grouping.AbstractRecordGrouping
			 * #outputRow(java.lang.String)
			 */
			@Override
			protected void outputRow(Object[] row) {
				if ("true".equals(row[3])) {
					countOfGroup = Integer.valueOf(row[row.length - 5]
							.toString());
			...
			protected void outputRow(RichRecord row) {
				List<DQAttribute<?>> originRow = row
						.getOutputRow(swooshGrouping.getOldGID2New());
		...
		survivorShipAlgorithmParams
				.setSurviorShipAlgos(new SurvivorshipFunction[] { func });
		...
		survivorShipAlgorithmParams
				.setDefaultSurviorshipRules(defaultSurvRules);
		...
		lnameRecords.put(IRecordGrouping.MATCHING_TYPE,
				AttributeMatcherType.DUMMY.name());
		lnameRecords.put(IRecordGrouping.CONFIDENCE_WEIGHT, String.valueOf(1));
		lnameRecords.put(IRecordGrouping.ATTRIBUTE_THRESHOLD,
				String.valueOf(0.9));
		lnameRecords.put(IRecordGrouping.RECORD_MATCH_THRESHOLD,
				String.valueOf(0.95f));
