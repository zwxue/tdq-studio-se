...
public final class EMFSharedResources {
	...
	public boolean saveSoftwareDeploymentResource() {
		return (softwareDeploymentResource != null) ? EMFUtil
				.saveSingleResource(softwareDeploymentResource) : false;
	}
	...
	/**
	 * Method "unloadResources" unloads and removes all the resources from the
	 * resource set.
	 */
	public synchronized void unloadResources() {
		List<Resource> resources = new ArrayList<Resource>(resourceSet
				.getResources());
	...
	/**
	 * Method "unloadResource" unload and remove the specification resource from
	 * the resource set.
	 * 
	 * @param uriString
	 *            the uri sting of resource.
	 */
	 ...
	 * 
	 * @param filePath
	 *            the file path to the resource which contains the given object
	 * @param eObject
	 *            an EMF object to save in the appropriate resource
	 * @return true when ok
	 */
	...
	public URI saveToUri(Resource res, URI destinationUri) {
		...
		// (inverse links)
		Map<EObject, Collection<Setting>> find = EcoreUtil.ExternalCrossReferencer
				.find(res);
