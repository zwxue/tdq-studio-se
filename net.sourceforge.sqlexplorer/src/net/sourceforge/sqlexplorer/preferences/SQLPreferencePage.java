...
public class SQLPreferencePage extends OverlaidPreferencePage implements IWorkbenchPreferencePage {
    ...
    private final Highlight[] highlights = new Highlight[] {
            new Highlight(Messages.getString("SQL_Table_1"), IConstants.SQL_TABLE), //$NON-NLS-1$
            new Highlight(Messages.getString("SQL_Column_2"), IConstants.SQL_COLUMS), //$NON-NLS-1$
            new Highlight(Messages.getString("SQL_Keyword_3"), IConstants.SQL_KEYWORD), //$NON-NLS-1$
            new Highlight(Messages.getString("SQL_Single_Line_Comment_4"), IConstants.SQL_SINGLE_LINE_COMMENT), //$NON-NLS-1$
            new Highlight(Messages.getString("SQL_Multi_Line_Comment_5"), IConstants.SQL_MULTILINE_COMMENT), //$NON-NLS-1$
            new Highlight(Messages.getString("String_6"), IConstants.SQL_STRING), //$NON-NLS-1$
            new Highlight(Messages.getString("Others_7"), IConstants.SQL_DEFAULT) }; //$NON-NLS-1$
    ...
    public SQLPreferencePage() {
        super(Messages.getString("Sql_Editor_Preferences_2"), GRID); //$NON-NLS-1$
    }
    ...
    protected void createFieldEditors() {
        try {
            // Font picker
            fontFieldEditor = new FontFieldEditor(IConstants.FONT, Messages.getString("Text_Font__3"), getFieldEditorParent()); //$NON-NLS-1$
            ...
            boldCheckBox.setText(Messages.getString("Bold_10")); //$NON-NLS-1$
            gd = new GridData(GridData.FILL_HORIZONTAL);
            gd.horizontalAlignment = GridData.BEGINNING;
            gd.horizontalSpan = 2;
            ...
            addField(new BooleanFieldEditor(IConstants.CLIP_EXPORT_COLUMNS, "Export column names", exportGroup)); //$NON-NLS-1$
        } catch (Exception e) {
            SQLExplorerPlugin.error(Messages.getString("SQLPreferencePage.NotCreateSQLPreferencePage"), e); //$NON-NLS-1$
    ...
    private Control createPreviewer(Composite parent) {
        ...
        final String content = Messages.getString("select_*_from_MyTable_--_single_line_comment_12") + separator + //$NON-NLS-1$
                Messages.getString("/*_multi_line_comment_13") + separator + //$NON-NLS-1$ 
                Messages.getString("select_*_14") + separator + //$NON-NLS-1$
                Messages.getString("end_multi_line_comment*/_15") + separator + //$NON-NLS-1$
                Messages.getString("where_A___1___16"); //$NON-NLS-1$
