...
/* package */abstract class MacroNameCommand extends Command {
    ...
    public MacroNameCommand(StructuredCommentParser parser, CommandType commandType, Token comment, Tokenizer tokenizer,
            CharSequence data) throws ParserException {
        super(parser, commandType, comment, tokenizer, data);
        if (tokens.size() == 0)
            throw new StructuredCommentException(Messages.getString("MacroNameCommand.MissMacroName", commandType), comment); //$NON-NLS-1$
        Token token = tokens.getFirst();
        if (token.getTokenType() != TokenType.WORD)
            throw new StructuredCommentException(Messages.getString("MacroNameCommand.MacroNamesValid"), comment); //$NON-NLS-1$
