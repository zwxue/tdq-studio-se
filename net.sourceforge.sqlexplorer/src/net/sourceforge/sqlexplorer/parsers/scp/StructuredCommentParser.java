/*
 * Copyright (C) 2006 SQL Explorer Development Team http://sourceforge.net/projects/eclipsesql
 * 
...
/**
 * Implements extensions to the underlying database platform via structured comments. Each structured comment begins
 * with ${ as the FIRST two characters in the comment - no space can occur between the start of the comment and the ${.
 * From there up to the first } is a command, and the rest of the comment is data for the command to act on.
 * 
 * The commands supported are:
 * 
 * ${define macro-name} value Defines a macro called macro-name and assigns a value
 * 
 * ${ifdef [!]macro-name} [data] If macro-name has been defined, then the data in the comment (IE the text outside the
 * ${...}) is uncommented. If the optional ! is provided then the macro-name must NOT have been defined. If data has not
 * been given, then ordinary code between ifdef and the next else or endif is commented depending on whether macro-name
 * is defined
 * 
 * ${else} [data] If the last structured comment was an ifdef which evaluated to false, then the data in the comment is
 * uncommented if
 * 
 * ${endif} Only needed to complete the multi-line versions of ifdef/else.
 * 
 * ${undef macro-name} Undefines the given macro name
 * 
 * ${ref macro-name} Causes the value of macro-name to be output instead of the comment; note that no error is raised if
 * macro-name has not been defined.
 * 
 * ${endref} Needed to complete multi-line versions of ref
 * 
 * ${parameter name [("output"|"inout")] [datatype] [arguments]} [value] Declares a named parameter, it's datatype
 * (decimal, int, string, or cursor), whether it is an output variable, and an optional value. If the datatype is
 * "cursor" then it is an output variable only; if not specified, it is input only. Parameters are specified in queries
 * using the notation ":name", eg "select * from people where age = :agetofind". If input or output is not specified,
 * then input is assumed (unless it is a cursor). Some datatypes have additional parameters - eg parameters of type
 * "date", where the format can be specified.
 * 
 * Up and coming: ${question [id=id] [datatype=(char|int|decimal|date)]} data Interactively asks a user a question
 * before running; the text of the question is in data, the default datatype is char. ID is used so that subsequent
 * executions can remember the last value typed for that ID.
 * 
 * ${date 'date-in-locale'} Replaced with parameter substitution to provide a date; date-in-locale is the date specified
 * in the client's default locale.
 * 
 * ${content-type column=column-name, type='mime-type'} Annotates a column named column-name as having the given
 * mime-type, overriding the default assumption by SQLExplorer. Typically intended for marking BLOBs as (for example)
 * images, can also be used to determine that varchars or CLOBs might contain XML, etc etc.
 * 
 * Examples: NOTE: in all of these examples, -- comments are used; the Java/C style of slash-asterisk to asterisk-slash
 * also works, but there's no examples here because Java will not understand it.
 * 
 * --${define DEBUG} true --${ifdef DEBUG} dbms_output.put_line('in debug mode...'); --${else} dbms_output.put_line('not
 * in debug mode'); --${endif}
 * 
 * Defines a macro called DEBUG, and outputs one of the dbms_output statements. If loaded through a standard SQL tool,
 * both statements would remain comments.
 * 
 * --${ifdef DEBUG} dbms_output.put_line('in debug mode'); dbms_output.put_line('The default is not in debug mode');
 * --${endif}
 * 
 * If DEBUG is defined, then the first dbms_output statement would be uncommented, but the second will become commented;
 * however if code was loaded through a standard SQL tool then the second line will run by default.
 * 
 * --${define DEFAULT_DATE} to_date('1980-JAN-01') ...snip... insert into mytable (id, when) values ( 1, --${ref
 * DEFAULT_DATE} );
 * 
 * This defines a macro called DEFAULT_DATE and assigns it the value "to_date('1980-JAN-01')"; later on it uses ref to
 * get the value of DEFAULT_DATE to insert into the second column
 * 
 * --${define SOME_SQL} select * from blah; ...snip... --${ref SOME_SQL} null; --${endref}
 * 
 * This defines SOME_SQL as "select * from blah;" and refers to it later. However, because the code uses
 * <code>endref</code>, then the non-comment between ref and endref is commented out; this allows a standard SQL tool to
 * still be able to compile with a default.
 * 
...
public class StructuredCommentParser {
    ...
    public enum CommandType {
        DEFINE {
            ...
            @Override
            public Command createInstance(StructuredCommentParser parser, Token comment, Tokenizer tokenizer, CharSequence data)
        ...
        public abstract Command createInstance(StructuredCommentParser parser, Token comment, Tokenizer tokenizer,
    ...
    /**
     * Constructor. <code>buffer</code> must be a writable buffer against which all tokens past to addComment have been
     * parsed.
     * 
     * @param buffer
     */
    ...
    public void addComment(Token comment) throws ParserException {
        ...
        /*
         * Conditional constructs (ie IFDEF, ELSE, and ENDIF) are linked together so that we can easily identify which
         * bits of code go through to the server
         */
        ...
        if (command instanceof ElseCommand) {
            ...
            if (last == null)
                throw new StructuredCommentException(Messages.getString("StructuredCommentParser.NoPreviousIfdef"), comment); //$NON-NLS-1$
            IfdefCommand ifdef = (IfdefCommand) last;
            if (ifdef.next != null)
                throw new StructuredCommentException(Messages.getString("StructuredCommentParser.alreadyEncountered"), comment); //$NON-NLS-1$
            ...
            if (last == null)
                throw new StructuredCommentException(Messages.getString("StructuredCommentParser.UnexpectedCommand"), comment); //$NON-NLS-1$
            PeeredCommand cond = (PeeredCommand) last;
            if (cond.next != null)
                throw new StructuredCommentException(
                        Messages.getString("StructuredCommentParser.endifAlreadyEncountered"), comment); //$NON-NLS-1$
            ...
            if (last == null || !(last instanceof RefCommand))
                throw new StructuredCommentException(Messages.getString("StructuredCommentParser.noPreceedRef"), comment); //$NON-NLS-1$
    ...
    public void process() {
        ...
        while (iter.hasNext()) {
            ...
            if (command.commandType == CommandType.DEFINE) {
                ...
                macros.put(def.macroName, def.data == null ? "" : def.data); //$NON-NLS-1$
    ...
    protected Command createCommand(Token comment) throws ParserException {
        ...
        int pos = sb.indexOf("}", 2); //$NON-NLS-1$
        if (sb.length() < 3 || !sb.substring(0, 2).equals("${") || pos < 0) //$NON-NLS-1$
            return null;
        ...
        if (token.getTokenType() != TokenType.WORD)
            throw new StructuredCommentException(Messages.getString(
                    "StructuredCommentParser.UnexpectedCommandInStructuredComment", token.toString()), comment); //$NON-NLS-1$
        ...
        try {
            ...
            throw new StructuredCommentException(Messages.getString(
                    "StructuredCommentParser.UnrecognisedStructuredComment", token.toString()), comment); //$NON-NLS-1$ 
        }
    ...
    /**
     * Deletes a section from the buffer and adjusts the offsets in the upcoming tokens accordingly; the section to be
     * deleted starts at the first character of startCmd and continues up to either the first character of endCmd if
     * endInclusive is false, or the last character of endCmd is endInclusive is true
     * 
