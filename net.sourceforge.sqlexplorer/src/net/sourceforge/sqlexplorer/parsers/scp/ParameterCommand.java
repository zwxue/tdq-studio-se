...
/* package */class ParameterCommand extends Command {
    ...
    public ParameterCommand(StructuredCommentParser parser, Token comment, Tokenizer tokenizer, CharSequence data)
        ...
        if (tokens.size() < 1)
            throw new StructuredCommentException(Messages.getString("ParameterCommand.noParameterName"), comment); //$NON-NLS-1$
        name = popWord();
        ...
        if (str != null)
            try {
                ...
                throw new StructuredCommentException(Messages.getString("ParameterCommand.UnrecognisedDataType", str), comment); //$NON-NLS-1$
            }
    ...
    protected String popWord() throws StructuredCommentException {
        ...
        if (token.getTokenType() != TokenType.WORD)
            throw new StructuredCommentException(Messages.getString("ParameterCommand.InvalidToken", token.toString()), comment); //$NON-NLS-1$
