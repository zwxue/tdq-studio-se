...
public enum EDriverName {
	...
	MYSQLDEFAULTURL("Mysql", //$NON-NLS-1$
			...
			"-6", "lib/mysql-connector-java-5.1.3-bin.jar"), HSQLDEFAULTURL(
			"HSql", //$NON-NLS-1$
			"org.hsqldb.jdbcDriver", //$NON-NLS-1$
			"-18", "lib/hsqldb.jar"), ORACLEDEFAULTURL("Oracle", //$NON-NLS-1$
			// MOD scorreia 2008-08-22: oracle.jdbc.driver package is not
			// supported anymore: replaced
			"oracle.jdbc.OracleDriver", //$NON-NLS-1$
			"-4", "lib/ojdbc14.jar"), MSSQLDEFAULTURL("MSSQL", //$NON-NLS-1$
			"net.sourceforge.jtds.jdbc.Driver", //$NON-NLS-1$
			"-11", "lib/jtds-1.2.jar"), //$NON-NLS-1$
	DB2DEFAULTURL(
			"DB2", //$NON-NLS-1$
			"com.ibm.db2.jcc.DB2Driver", //$NON-NLS-1$
			"-24", "lib/db2jcc_license_cu.jar", "lib/db2jcc_license_cisuz.jar",
			"lib/db2jcc.jar"), POSTGRESQLEFAULTURL("PostgreSQL", //$NON-NLS-1$
			"org.postgresql.Driver", //$NON-NLS-1$
			"-7", "lib/postgresql-8.1-405.jdbc3.jar"), INTERBASEDEFAULTURL(
			"Interbase", //$NON-NLS-1$
			"interbase.interclient.Driver", //$NON-NLS-1$
			"-3", "lib/interclient.jar"), SYBASEDEFAULTURL("Sybase", //$NON-NLS-1$
			"com.sybase.jdbc3.jdbc.SybDriver", //$NON-NLS-1$
			"-9", "lib/jconn3.jar"), INFORMIXDEFAULTURL("Informix", //$NON-NLS-1$
			"com.informix.jdbc.IfxDriver", //$NON-NLS-1$
			null, "lib/ifxjdbc.jar"), //$NON-NLS-1$ 
	FIREBIRDDEFAULTURL("FireBird", //$NON-NLS-1$
			"org.firebirdsql.jdbc.FBDriver", //$NON-NLS-1$
			"-25", "lib/jaybird-2.1.1.jar"), TERADATADEFAULTURL("Teradata",
					...
					for (String jar : jars) {
						...
						for (ManifestElement element : elements) {
							if (jar.equals(element.getValue())) {
								...
								hsqldbJar = FileLocator.toFileURL(bundle
										.getEntry(value));
