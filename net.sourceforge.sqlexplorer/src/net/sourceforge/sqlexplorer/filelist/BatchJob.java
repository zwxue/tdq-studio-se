...
public class BatchJob extends Job {
    ...
    public BatchJob(User user, List<File> files) {
        this(Messages.getString("BatchJob.Title"), user, files); //$NON-NLS-1$
    }
    ...
    protected IStatus run(IProgressMonitor monitor) {
        monitor.beginTask(Messages.getString("BatchJob.ExecutingScripts"), files.size()); //$NON-NLS-1$
            ...
            for (File file : files) {
                ...
                try {
                    ...
                    if (length < 0 || length >= buffer.length) {
                        SQLExplorerPlugin.error(Messages.getString("BatchJob.NotReadFromFile", file.getAbsolutePath())); //$NON-NLS-1$
                        continue;
                    }
                    ...
                    SQLExplorerPlugin.error(Messages.getString("BatchJob.CannotReadFromFile", file.getAbsolutePath()), e); //$NON-NLS-1$
                    continue;
                }
        ...
        return new Status(IStatus.OK, getClass().getName(), IStatus.OK, Messages.getString("BatchJob.Success"), null); //$NON-NLS-1$
    }
