    ...
    public SQLConnection getConnection(User user) throws SQLException {
        ...
        if (!isDriverClassLoaded())
            try {
                ...
                throw new SQLException(Messages.getString("ManagedDriver.notLoadJDBCdriver", //$NON-NLS-1$
                        driverClassName));
            }
        if (!isDriverClassLoaded())
            throw new SQLException("Cannot load JDBC driver " + driverClassName); //$NON-NLS-1$
