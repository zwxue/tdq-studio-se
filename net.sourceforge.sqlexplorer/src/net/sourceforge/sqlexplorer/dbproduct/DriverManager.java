...
/**
 * Provides access to the list of drivers, persisting their configuration in the Eclipse workspace; restoring to
 * defaults is also supported.
 * 
 * This is part of the rewrite of SQLAlias, which was originally taken from SquirrelSQL; the old DriverModel (via
 * DataCache) used parts of Squirrel which no longer exist (even in the SquirrelSQL CVS on Sourceforge) and are
 * effectively undocumented. Changes needed to fix bugs relating to transactions and multiple logons per alias meant
 * that keeping the old code became unmaintainable, hence the sweeping rewrite.
 * 
...
public class DriverManager {
    ...
    public static final String DRIVER_CLASS = "driver-class"; //$NON-NLS-1$

    public static final String DRIVERS = "drivers"; //$NON-NLS-1$

    public static final String ID = "id"; //$NON-NLS-1$

    public static final String JARS = "jars"; //$NON-NLS-1$
    ...
    public void restoreDrivers() throws ExplorerException {
        try {
            drivers.clear();
            highestId = 0;
            URL url = URLUtil.getResourceURL("default_drivers.xml"); //$NON-NLS-1$
    ...
    public void loadDrivers() throws ExplorerException {
        try {
            ...
            throw new ExplorerException(Messages.getString("DriverManager.CannotLoadDrivers", e.getMessage()), e); //$NON-NLS-1$
        }
    }
    ...
    public void addDriver(ManagedDriver driver) {
        if (driver.getId() == null || driver.getId().trim().length() == 0)
            throw new IllegalArgumentException(Messages.getString("DriverManager.invalidId")); //$NON-NLS-1$
        if (drivers.get(driver.getId()) != null)
            throw new IllegalArgumentException(Messages.getString("DriverManager.idExists", driver.getId())); //$NON-NLS-1$
    ...
    protected Element convertFromV3(Element root) {
        ...
        for (Element elem : root.elements("Bean")) { //$NON-NLS-1$
            String str;
            Element driver = result.addElement(DRIVER);

            try {
                str = elem.element("identifier").elementText("string"); //$NON-NLS-1$ //$NON-NLS-2$
                driver.addAttribute(ID, str);

                str = elem.elementText("driverClass"); //$NON-NLS-1$
                if (str != null)
                    driver.addElement(DRIVER_CLASS).setText(str);

                str = elem.elementText("name"); //$NON-NLS-1$
                driver.addElement(NAME).setText(str);

                str = elem.elementText("url"); //$NON-NLS-1$
                driver.addElement(URL).setText(str);

                Element jars = driver.addElement(JARS);
                Element jarFileNames = elem.element("jarFileNames"); //$NON-NLS-1$
                for (Element jarBeanElem : jarFileNames.elements("Bean")) { //$NON-NLS-1$
                    str = jarBeanElem.elementText("string"); //$NON-NLS-1$
                    if (str != null && str.trim().length() > 0)
                        jars.addElement(JAR).setText(str);
                }
            } catch (IllegalArgumentException e) {
                SQLExplorerPlugin.error(Messages.getString("DriverManager.ErrorLoadDriver", driver.attributeValue(ID)), e); //$NON-NLS-1$
                throw e;
            }
        }
