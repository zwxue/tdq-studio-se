...
public class NewDatabaseStructureViewAction extends AbstractConnectionTreeAction {
    ...
    public NewDatabaseStructureViewAction() {
        super("ConnectionsView.Actions.NewDatabaseStructure", "ConnectionsView.Actions.NewDatabaseStructure.Tooltip", //$NON-NLS-1$ //$NON-NLS-2$
                "Images.NewDatabaseStructure"); //$NON-NLS-1$
    }
        ...
        for (User user : users)
            try {
                ...
                MessageDialog.openError(Display.getDefault().getActiveShell(), Messages
                        .getString("NewDatabaseStructureViewAction.CannotConnect"), e.getMessage()); //$NON-NLS-1$
            }
    }
