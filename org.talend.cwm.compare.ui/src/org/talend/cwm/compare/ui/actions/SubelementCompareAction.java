...
public class SubelementCompareAction extends Action {
    ...
    public void run() {
        ...
        if (selection.toList().size() == 1) {
            ...
            if (selectedElement instanceof ColumnSet) {
                ...
                PackageHelper.addColumnSet((ColumnSet) selectedElement, parentPackage);
                ...
                // Judge and see if there are schemas under the catalog.(Case of SQL Servers)
                List<TdSchema> schemas = CatalogHelper.getSchemas((TdCatalog) selectedElement);
    ...
    private IFolderNode getTableOrViewFolder(EObject selectedElement) {
        IFolderNode folderNode = null;
        TdDataProvider dataProvider = PrvResourceFileHelper.getInstance().findProvider((IFile) selectedOjbect).getObject();
        ((Package) selectedElement).getDataManager().add(dataProvider);
        ModelElement matchedElement = findMatchedModelElement(dataProvider, selectedElement);
    ...
    private ModelElement findMatchedModelElement(EObject parent, EObject similarElement) {
        if (parent instanceof TdDataProvider) {
            ...
            for (TdCatalog catalog : catalogs) {
                ...
                TdCatalog ctl = CatalogHelper.getParentCatalog((ModelElement) similarElement);
                if (!ctl.getName().equalsIgnoreCase(catalog.getName())) {
                    continue;
                }
