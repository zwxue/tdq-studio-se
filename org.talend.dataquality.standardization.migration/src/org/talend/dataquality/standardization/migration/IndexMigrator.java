...
/**
 * A tool to regenerate all "out of the box" indexes with specified analyzer.
 * The regeneration simply reads and re-writes all detected indexes in
 * inputPath. This class is independent from SynonymIndexBuilder class.
 * 
 * @author sizhaoliu
 */
public class IndexMigrator {
	...
	public int run() throws IOException {
		...
		if (!inputFolder.exists() || !inputFolder.isDirectory()) {
			System.err.println("The input path <" + inputPath
					+ "> does not exist or is not a folder.");
			System.err
					.println("Usage: java -jar IndexMigrator.jar <inputPath> <outputPath(optinal)>");
			return -1;
		}
		...
		if ("".equals(outputPath)) {
			System.out
					.println("No output folder specified. The new index(es) will be genenrated in <"
							+ inputPath + "_REGENERATED> folder");
			outputFolder = new File(inputPath + "_REGENERATED");
		} else {
			outputFolder = new File(outputPath);
		}
		if (outputFolder.exists() && outputFolder.isDirectory()) {
			System.out.println("The path <" + outputFolder
	 ...
	 * @throws IOException
	 */
	private int regenerate(File inputFolder, File outputFolder)
			throws IOException {
		...
		if (status.missingSegments) {
			for (File f : inputFolder.listFiles()) {
				if (f.isDirectory()) {
					File out = new File(outputFolder.getAbsolutePath() + "/"
							+ f.getName());
					out.mkdir();
					regenerate(f, out);
				}
			...
			IndexWriter writer = new IndexWriter(outputDir, analyzer,
					IndexWriter.MaxFieldLength.UNLIMITED);

			IndexSearcher searcher = new IndexSearcher(indexDir);

			Collection<String> fieldNames = searcher.getIndexReader()
			...
			for (File file : inputFolder.listFiles()) {
				if (!isLuceneIndexFile(file)) {
					// copy to destination folder
					copyFile(file, new File(outputFolder.getAbsolutePath()
							+ "/" + file.getName()));
				}
			}
		}
	...
	private Document generateDocument(String word, Set<String> synonyms) {
		...
		word = word.trim();
		Field wordField = new Field(F_WORD, word, Field.Store.YES,
				Field.Index.ANALYZED_NO_NORMS, TermVector.NO);
		doc.add(wordField);
		Field wordTermField = new Field(F_WORDTERM, word.toLowerCase(),
		...
		for (String syn : synonyms) {
			if (syn != null) {
				...
				if (syn.length() > 0 && !syn.equals(word)) {
					doc.add(new Field(F_SYN, syn, Field.Store.YES,
							Field.Index.ANALYZED_NO_NORMS, TermVector.NO));
					doc.add(new Field(F_SYNTERM, syn.toLowerCase(),
	...
	/**
	 * check if a file is for Lucene index. A complete list of lucene index
	 * formats can be found here:
	 * 
	 * http://lucene.apache.org/core/old_versioned_docs/versions/3_0_1/
	 * fileformats.html
	 * 
	 * @param file
	 */
	private boolean isLuceneIndexFile(File file) {
		...
		if (fileName.startsWith("segments") || "write.lock".equals(fileName)
				|| fileName.endsWith(".cfs") || fileName.endsWith(".fnm")
				|| fileName.endsWith(".fdx") || fileName.endsWith(".fdt")
				|| fileName.endsWith(".tis") || fileName.endsWith(".tii")
				|| fileName.endsWith(".frq") || fileName.endsWith(".prx")
				|| fileName.endsWith(".nrm") || fileName.endsWith(".tvx")
				|| fileName.endsWith(".tvd") || fileName.endsWith(".tvf")
